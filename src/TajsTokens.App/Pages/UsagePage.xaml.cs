...
public sealed partial class UsagePage : Page
{
    ...
    private string? _model, _repository, _session;
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            ...
            { UsageOnly = true, Model = _model, Repository = _repository, SessionId = _session };
            ...
            _rows = time
                ...
                : data.Dimensions.Where(x => x.Dimension == _group).Select(x => new UsageRow(x.Value, x.NativeTokens, x.UncachedInputTokens, x.NonReasoningOutputTokens, x.CacheReadTokens, x.CacheWriteTokens, x.ReasoningOutputTokens, x.Sessions)).ToArray();
            ...
            var filters = string.Join(" · ", new[] { _model, _repository, _session }.Where(x => x is not null));
        ...
        {
            if (_loaded && _request == request) StatusText.Text = "Usage unavailable: " + e.Message;
        }
    ...
    {
        _model = _repository = _session = null; _from = _to = null; SearchBox.Text = ""; await LoadAsync();
    }
    private void OnRowSelected(object sender, SelectionChangedEventArgs e)
    {
        ...
        OpenThreadButton.IsEnabled = row is not null && _group == "Session";
    ...
    {
        if (_group == "Session" && RowsList.SelectedItem is UsageRow row)
            ThreadRequested?.Invoke(this, row.Label);
    }
    ...
    private sealed record UsageRow(string Label, long Total, long Input, long Output, long Cache, long Write, long Reasoning, int Sessions, DateTimeOffset? Start = null, DateTimeOffset? End = null)
    {
