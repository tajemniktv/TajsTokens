// Taj's Tokens | ForecastsPage.xaml.cs
...
public sealed partial class ForecastsPage : Page
{
    ...
    public ForecastsPage()
    {
        ...
        SizeChanged += (_, e) =>
        {
            ...
            Grid.SetColumn(HistoryDetails, wide ? 1 : 0);
            Grid.SetRow(HistoryDetails, wide ? 0 : 1);
        };
    }
    ...
    protected override void OnNavigatedTo(NavigationEventArgs e)
    {
        ...
        object? normalTab = _normalTab;
        _modelLab = Equals(e.Parameter, "model-lab");
        ForecastTabs.Items.Clear();
        foreach (PivotItem tab in _modelLab ? new[] { EvaluationTab } : new[] { OutlookTab, PlanTab, UsageTab, HistoryTab })
    ...
    private void OnModelLabClicked(object sender, RoutedEventArgs e)
    {
        App.Navigate(typeof(ForecastsPage), "model-lab");
    }
    ...
    private async Task LoadAsync(CancellationToken cancellationToken, long generation)
    {
        ...
        try
        {
            ...
            int days = ParseHistoryDays();
            IntelligenceDashboard dashboard = await Task.Run(
                () => App.Services.Intelligence.QueryAsync(
                    new IntelligenceQuery(now.AddDays(-days), now, AnalyticsBucketSize.Day),
                    cancellationToken),
                cancellationToken);
    ...
    private void RenderForecasts(IntelligenceDashboard dashboard)
    {
        ...
        TelemetrySnapshot snapshot = App.Services.Telemetry.Latest;
        QuotaSnapshot[] current = snapshot.QuotaSnapshots.Where(x => x.Kind is QuotaWindowKind.FiveHour or QuotaWindowKind.Weekly)
        ...
        {
            CurrentQuotaForecast? forecast = snapshot.FindCurrentForecast(x);
            return QuotaCardPresenter.BuildQuotaCard(
                x.Kind == QuotaWindowKind.Weekly ? "Weekly quota" : "5-hour quota",
                x,
                forecast is { IsFresh: true } ? forecast.Forecast : null,
                snapshot.IsQuotaSnapshotFresh(x));
        }).ToArray();
        CurrentQuotaForecast?[] trusted = current.Select(snapshot.FindCurrentForecast).Where(x => x is { IsFresh: true }).ToArray();
        bool advanced = trusted.Any(x =>
            x?.Forecast?.Evidence?.HorizonPredictions?.Any(h => h.Model.StartsWith("composed-quota/", StringComparison.Ordinal)) == true);
        ModelHealthText.Text = current.Length == 0
            ? "No current quota reading. Refresh Codex quota before relying on an outlook."
            :
            trusted.Length == 0
                ? "No fresh outlook is available. Refresh quota and check Diagnostics before relying on a forecast."
                :
                advanced
                    ?
                    "A workload-based short-term prediction has earned live selection. The outlook above remains conditional on your future activity."
                    :
        ...
        TokenWorkloadForecast? tokens = snapshot.TokenForecast;
        ...
        TokenPredictionRange.Text = tokens is not null
            ? string.Join(
                ...
                tokens.Predictions.Select(x => x.LowerTokens is { } low && x.UpperTokens is { } high
                    ...
                    : $"{Horizon(x.HorizonHours)}: uncertainty still learning"))
            : string.Empty;
            ...
            ? $"{tokens.Sessions:N0} sessions · {tokens.TokenEvents:N0} token observations · generated {tokens.GeneratedAtUtc.ToLocalTime():g}\n\n" +
              string.Join(
                  ...
                  tokens.Predictions.Select(x => $"+{x.HorizonHours:0.#}h: {x.Explanation}" +
                                                 (x.LowerTokens is { } low && x.UpperTokens is { } high
                                                     ? $" Range: {low:N0}–{high:N0} tokens."
                                                     : "") +
                                                 (x.Composition is { } c
                                                     ...
                                                     : ""))) +
              "\n\n" + tokens.Methodology + "\n\n" + SessionWorkloadPredictionService.Methodology
    ...
    {
        ForecastSnapshot? latest = _history.FirstOrDefault(x => x.Forecast.Kind == kind);
        QuotaLaneState? lane = App.Services.Telemetry.Latest.QuotaLanes.FirstOrDefault(x => x.Kind == kind);
        ...
        caption.Text = lane?.NotReportedByProvider == true
            ? "This window is absent from the current provider response. Saved projections remain in history below."
            : latest is null
                ? "History will appear after quota observations are collected."
    ...
    {
        (string Provider, string Profile, QuotaWindowKind Kind, DateTimeOffset GeneratedAtUtc, string? AccountKey)? selectedIdentity =
            (ForecastList.SelectedItem as ForecastRow)?.Identity;
        ForecastSnapshot[] filtered = _history.Where(x => WindowFilter.SelectedIndex == 0 ||
                                                          x.Forecast.Kind == (WindowFilter.SelectedIndex == 1
                                                              ? QuotaWindowKind.FiveHour
                                                              : QuotaWindowKind.Weekly)).ToArray();
        ForecastSnapshot[] visible = HistoryDensity.SelectedIndex == 0
            ...
            : filtered;
        ForecastRow[] rows = visible.Select(snapshot =>
        {
            Forecast f = snapshot.Forecast;
            string details = $"Pace: {(f.BurnRatePercentPerHour is double rate ? $"{rate:0.##} quota points/hour" : "not established")}\n" +
                             ...
                             (f.Evidence?.HorizonPredictions is { Count: > 0 } horizons
                                 ? string.Join(
                                       "\n\n",
                                       horizons.Select(x =>
                                           $"+{x.HorizonHours:0.#}h: ~{x.RemainingPercent:0.#}% remaining · {x.Model}\n{x.Explanation}")) +
                                   "\n\n"
                                 : "") +
            ...
            string summary = $"{QuotaAccountScope.Describe(snapshot.AccountKey)}\n" +
                                 ...
                                 ? string.Join(
                                     ...
                                     outlooks.Select(x => $"In {Horizon(x.HorizonHours)}: ~{x.RemainingPercent:0.#}% remaining" +
                                                          (x.LowerRemainingPercent is { } low && x.UpperRemainingPercent is { } high
                                                              ? $" · 80%-target range {low:0.#}–{high:0.#}%"
                                                              : " · uncertainty learning")))
                                 : f.Evidence?.UncertaintyDescription ?? "No uncertainty method recorded.");
            return new ForecastRow(
                $"{f.GeneratedAtUtc.ToLocalTime():g} · {FormatKind(f.Kind)}",
                FormatOutcome(f),
                details,
                (snapshot.Provider, snapshot.Profile, f.Kind, f.GeneratedAtUtc, snapshot.AccountKey),
                QuotaAccountScope.Describe(snapshot.AccountKey),
                summary);
        }).ToArray();
    ...
    private static string FormatOutcome(Forecast f)
    {
    ...
    private async void OnLoadTtHistoryClicked(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            IReadOnlyList<TtEvaluationArchiveEntry> history =
                await App.Services.Intelligence.GetTtEvaluationHistoryAsync("codex", "default", 10, cancellation.Token);
            if (!_isLoaded || cancellation.IsCancellationRequested) return;
            TtHistoryText.Text = history.Count == 0
                ? "No saved TT research results yet. Run Model evaluation to save a result locally."
        ...
        catch (Exception exception)
        {
            if (_isLoaded && !cancellation.IsCancellationRequested)
                TtHistoryText.Text = "Saved TT history unavailable: " + Summarize(exception.Message);
        }
    }
    ...
    private async void OnEvaluateClicked(object sender, RoutedEventArgs e)
    {
        CancellationTokenSource? cancellation = _pageCancellation;
        if (!_isLoaded || cancellation is null || cancellation.IsCancellationRequested) return;
        long generation = Interlocked.Increment(ref _evaluationGeneration);
        ...
        {
            DateTimeOffset now = DateTimeOffset.UtcNow;
            DateTimeOffset from = now.AddDays(-ParseHistoryDays());
            ForecastEvaluationReport report = await Task.Run(
                () => App.Services.Intelligence.EvaluateForecastsAsync(
                    "codex",
                    "default",
                    from,
                    now,
            ...
            EvaluationStatusText.Text =
                $"Evaluated {from.ToLocalTime():d}–{now.ToLocalTime():d} · {report.Scores.Count:N0} quota comparisons · {report.TokenScores.Count:N0} token comparisons. Select a cohort below; lower error is better within the same target.";
            EvaluationMethodText.Text =
                $"{report.QuotaHistorySummary} {report.WorkloadObservations:N0} native workload observations · {report.TokensWithEffort:N0}/{report.TokenObservations:N0} token records with effort. {report.Methodology}";
            if (report.QuotaCost is { } cost)
            {
                ...
                if (cost.EvidenceCoverage is { } coverage)
                    EvaluationMethodText.Text +=
                        ...
                        "Requested-tier settings: " +
                        string.Join(", ", coverage.RequestedTierSettings.Select(x => $"{x.Key}={x.Value:N0}")) +
            ...
            if (report.Tt is { } tt)
                EvaluationMethodText.Text += "\n\n" + tt.Methodology +
                                             (tt.Scores.Any(x => x.IsTransfer)
                                                 ? ""
                                                 : " No chronological compatible TT transfer pairs in this range; cross-regime scaling remains unvalidated.");
            if (report.SavedTtSnapshotId is { } savedTtId)
                EvaluationMethodText.Text +=
                    $"\nOriginal TT research snapshot saved locally: {savedTtId}. Later runs are separate reconstructions, not updates to this result.";
            if (report.QuotaTransfer is { } transfer)
                EvaluationMethodText.Text += "\n\n" + transfer.Methodology +
                                             (transfer.Scores.Count == 0
                                                 ? " No compatible recorded-account regimes in this range; transfer and TT remain unsupported."
                                                 : "");
            if (report.SessionScores.Count > 0)
                EvaluationMethodText.Text += "\n\n" + SessionWorkloadPredictionService.Methodology + "\n" +
                                             string.Join(
                                                 "\n",
                                                 report.SessionScores.Select(x =>
                                                     $"Session {Horizon(x.HorizonHours)}: {x.Origins} activity outcomes ({x.ActiveOrigins} positive); Brier {x.BrierScore:0.000} vs global frequency {x.BaselineBrierScore:0.000}; calibration error {x.CalibrationError:0.000}. " +
            ...
            _evaluationRows = report.Scores.Select(score => new EvaluationRow(
                    ...
                    score.Origins == 0
                        ...
                        : $"Mean error {score.MeanAbsoluteError:0.##} quota points · {score.Origins:N0} origins · {score.ResetGenerations} reset generations",
                    score.Origins == 0
                        ? "No eligible outcomes in this range."
                        : $"MAE {score.MeanAbsoluteError:0.##}pp · RMSE {score.RootMeanSquaredError:0.##}pp · {score.Availability} · {QuotaHistoryPolicy.DescribeCohort(score.HistoryCohort)}" +
                          (score.Model.EndsWith("ridge", StringComparison.Ordinal)
                              ? $" · {score.FittedOrigins} fitted origins (others use baseline)"
                              : string.Empty) +
                          (score.IntervalOrigins > 0
                              ? $" · band coverage {score.IntervalCoverage:P0} on {score.IntervalOrigins} origins; width {score.MeanIntervalWidth:0.#}pp"
                              : " · insufficient interval calibration") +
                          ...
                          (score.ExhaustionClassificationAccuracy is double accuracy
                              ? $" · classification accuracy {accuracy:P0}"
                              : string.Empty) +
                          (score.EtaBracketMeanAbsoluteHours is double etaError
                              ? $" · ETA bracket error {etaError:0.##}h on {score.EtaOrigins} origins"
                              : string.Empty)))
                .Concat(
                    report.TokenScores.Select(score => new EvaluationRow(
                        ...
                        (score.IntervalOrigins > 0
                            ? $" · band coverage {score.IntervalCoverage:P0} on {score.IntervalOrigins} origins"
                            : ""))))
                .Concat(
                    (report.QuotaCost?.ConstructionCoverage ?? []).Select(coverage => new EvaluationRow(
                ...
                .Concat(
                    (report.QuotaCost?.CohortCoverage ?? []).Select(coverage => new EvaluationRow(
                ...
                .Concat(
                    (report.QuotaCost?.Scores ?? []).Select(score => new EvaluationRow(
                        ...
                        (score.BandSamples > 0
                            ? $"Band/target intersection {score.BandIntersectsTargetRate:P0} on {score.BandSamples} intervals; not latent coverage. "
                            : "Insufficient generations for bands. ") +
                ...
                .Concat(
                    (report.ComposedQuota?.Scores ?? []).Concat(report.ComposedQuotaStrict?.Scores ?? []).Select(score => new EvaluationRow(
                        ...
                        score.CostModel,
                        score.WithheldReasons.ContainsKey("no-recorded-training-work")
                            ?
                            "Cost unavailable: no recorded workload in frozen training"
                            :
                            score.HeldOutIntervals == 0
                        ...
                        ComposedQuotaPolicy.AssessSelection(
                            score,
                        ...
                        (score.IntervalOrigins > 0
                            ? $"Joint range: {score.IntervalCoverage:P0} reported-value coverage on {score.IntervalOrigins} origins; mean width {score.MeanIntervalWidth:0.###}pp. "
                        ...
                        (score.WithheldReasons.Count == 0
                            ? ""
                            : "Reasons (may overlap): " + string.Join(
                                "; ",
                                score.WithheldReasons.Select(x => $"{x.Key}: {x.Value}")) + ". ") +
                        (score.Availability == ForecastReplayAvailability.CollectedByOrigin
                            ? report.ComposedQuotaStrict!.Methodology
                            : report.ComposedQuota!.Methodology) +
                        "\n\n" + FormatComposedBreakdowns(score))))
                .Concat(
                    (report.SessionQuota?.Scores ?? []).Select(score => new EvaluationRow(
                        ...
                        (score.TrainingIssue is { } issue
                            ? $"Cost training unavailable: {issue}. No recorded work is not evidence of free work or complete collection. "
                            : "") +
                        ...
                        (score.BandOrigins > 0
                            ? $"Joint band coverage {score.ReportedBandCoverage:P0} on {score.BandOrigins} outcomes; mean width {score.MeanBandWidth:0.###}pp. "
                            : "Insufficient completed resets for joint bands. ") +
                        report.SessionQuota!.Methodology)))
                .Concat(
                    (report.Tt?.Scores ?? []).Select(score => new EvaluationRow(
                        ...
                        FormatTtBias(score) +
                        (score.Basis is { } basis
                            ? "Category weights/token: " + string.Join(
                                  ", ",
                                  basis.Weights.Select(x => x.ToString("G6", CultureInfo.InvariantCulture))) +
                              "; 1-TT reference category counts: " + string.Join(
                                  ", ",
                                  basis.Reference.Select(x => x.ToString("G6", CultureInfo.InvariantCulture))) +
                              "; supported models: " + string.Join(", ", basis.Models) + "; efforts: " + string.Join(", ", basis.Efforts) +
                              ". "
                            : "") +
                        report.Tt!.Methodology)))
                .Concat(
                    (report.QuotaTransfer?.Scores ?? []).Select(score => new EvaluationRow(
    ...
    private void OnEvaluationGroupChanged(object sender, SelectionChangedEventArgs e)
    {
        RenderEvaluationGroup();
    }
    ...
    private void RenderEvaluationGroup()
    {
        ...
        string? group = EvaluationGroup.SelectedItem as string;
        EvaluationRow[] rows = _evaluationRows.Where(x => x.Group == group).ToArray();
        EvaluationList.ItemsSource = rows;
        EvaluationSelectionText.Text = group is null
            ? "No evaluation results in this range."
    ...
    private static string FormatComposedBreakdowns(ComposedQuotaScore score)
    {
        ...
        {
            return value is { } n ? $"{n:0.###}" : "unavailable";
        }
        ...
        return "Held-out diagnostic partitions (not model selection):\n" + string.Join(
                   ...
                   score.Breakdowns.Take(60).Select(x =>
                       ...
                       (x.BandOutcomes > 0
                           ? $"Band coverage {x.BandCoverage:P0}, width {x.MeanBandWidth:0.###}pp ({x.BandOutcomes})."
                           : "No evaluated band outcomes."))) +
    ...
    {
        ComposedQuotaScore[] scores = report.ComposedQuotaStrict?.Scores.Where(x => x.Cohort == coverage.Cohort &&
                                                                                    x.HorizonHours == coverage.HorizonHours).ToArray() ??
                                      [];
        return scores.Length == 0
            ? "Strict replay: no matching comparison; not evidence of zero exclusions."
            : "Strict replay (per candidate; counts overlap, do not sum): " + string.Join(
                " | ",
                scores.Select(x =>
    ...
    private static string FormatTtBias(TtEvaluationScore score)
    {
        return score.ScalarBias is null
    ...
    private static string Horizon(double hours)
    {
        return hours < 1 ? $"{hours * 60:0} min" : $"{hours:0.#}h";
    }

    private static string CompactTokens(double? value)
    {
        return value switch
        {
    ...
    private async void OnRecentPatternClicked(object sender, RoutedEventArgs e)
    {
        CancellationTokenSource? cancellation = _pageCancellation;
        if (!_isLoaded || cancellation is null || cancellation.IsCancellationRequested) return;
        (double, double, string, string) original = (RootAgentsBox.Value, SubagentsBox.Value, ModelBox.Text, ReasoningBox.Text);
        RecentPatternButton.IsEnabled = false;
        RecentPatternText.Text = "Reading recent local activity…";
        try
        {
            RecentScenarioPattern pattern =
                await App.Services.Intelligence.GetRecentScenarioPatternAsync(DateTimeOffset.UtcNow, cancellation.Token);
            if (!_isLoaded || cancellation.IsCancellationRequested) return;
            ...
            {
                RecentPatternText.Text =
                    "Recent session counts exceed this planner's input range; no values were silently clamped. Enter a hypothetical workload manually.";
                return;
            }
        ...
        {
            if (_isLoaded && !cancellation.IsCancellationRequested)
                RecentPatternText.Text = "Recent pattern unavailable: " + Summarize(exception.Message);
        }
        finally
        {
            RecentPatternButton.IsEnabled = true;
        }
    }
    ...
    private async void OnEstimateClicked(object sender, RoutedEventArgs e)
    {
        ...
        long generation = Interlocked.Increment(ref _estimateGeneration);
        try
        {
            string?[] accounts = App.Services.Telemetry.Latest.QuotaLanes.Where(lane => lane.IsFresh)
            ...
            if (accounts.Length != 1 || accounts[0] is null)
            {
                ...
                ScenarioInfo.Message =
                    "Refresh quota and retry. A scenario needs a fresh Codex response with a known backend account; saved history alone cannot establish the current account.";
                ScenarioMethodText.Text = string.Empty;
                return;
            }
            var request = new ScenarioRequest(
                ...
                accounts[0]);
            DateTimeOffset historyFrom = DateTimeOffset.UtcNow.AddDays(-ParseHistoryDays());
            ScenarioInfo.IsOpen = true;
            ScenarioInfo.Severity = InfoBarSeverity.Informational;
            ...
            ScenarioEstimate estimate = await Task.Run(
            ...
            bool bothReady = estimate.FiveHour.HasEnoughHistory && estimate.Weekly.HasEnoughHistory;
            bool anyReady = estimate.FiveHour.HasEnoughHistory || estimate.Weekly.HasEnoughHistory;
    ...
    {
        if (HistoryRangeCombo.SelectedItem is ComboBoxItem item && int.TryParse(item.Tag?.ToString(), out int days))
        {
    ...
    private static string FormatEstimate(ScenarioWindowEstimate estimate)
    {
        ...
        string range = estimate.LowerQuotaDeltaPercent is not null && estimate.UpperQuotaDeltaPercent is not null
    ...
    {
        ForecastSnapshot[] decided = snapshots.Where(snapshot => snapshot.Forecast.SurvivesUntilReset is not null).ToArray();
        if (decided.Length == 0)
        {
            return "Learning";
        }
        int safe = decided.Count(snapshot => snapshot.Forecast.SurvivesUntilReset == true);
