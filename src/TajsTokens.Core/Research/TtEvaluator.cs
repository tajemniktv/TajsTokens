...
public static class TtEvaluator
{
    ...
    public static TtEvaluation Evaluate(IReadOnlyList<QuotaCostObservation> rows, CancellationToken cancellationToken = default)
    {
        ...
        (QuotaHistoryCohort Cohort, double HorizonHours, QuotaCostObservation[] Rows)[] groups = rows
            .Where(x => x.Cohort.AccountKey is not null).GroupBy(x => (x.Cohort, x.HorizonHours))
            .Select(x => (x.Key.Cohort, x.Key.HorizonHours, Rows: x.OrderBy(y => y.StartUtc).ToArray())).ToArray();
        IEnumerable<((QuotaHistoryCohort Cohort, double HorizonHours, QuotaCostObservation[] Rows) Source, (QuotaHistoryCohort Cohort,
            double HorizonHours, QuotaCostObservation[] Rows) Destination, bool Transfer)> comparisons = groups
            .Select(g => (Source: g, Destination: g, Transfer: false)).Concat(
                ...
                where source.Cohort != destination.Cohort && source.HorizonHours == destination.HorizonHours &&
                      ...
                      source.Cohort.SessionId == destination.Cohort.SessionId &&
                      source.Rows.Max(x => x.EndUtc) <= destination.Rows[0].StartUtc
                select (Source: source, Destination: destination, Transfer: true));
        foreach (((QuotaHistoryCohort Cohort, double HorizonHours, QuotaCostObservation[] Rows) Source, (QuotaHistoryCohort Cohort, double
                 HorizonHours, QuotaCostObservation[] Rows) Destination, bool Transfer) comparison in comparisons)
        {
            cancellationToken.ThrowIfCancellationRequested();
            QuotaCostObservation[] ordered = comparison.Destination.Rows;
            QuotaCostObservation[] training = comparison.Source.Rows.Take(20).ToArray();
            ...
            {
                double[] weights = QuotaAccountingModel.FitCategoryWeights(training, cancellationToken);
                double[] basket = reference.TokenCategories.Select(x => x / reference.TokenCategories.Sum() * 1e6).ToArray();
                if (basket.Select((x, i) => x * weights[i]).Sum() > 0)
                    basis = new TtWorkloadBasis(
                        weights,
                        basket,
                        Enumerable.Range(0, 5).Select(i => training.Any(x => x.TokenCategories[i] > 0)),
                        training.SelectMany(x => x.Features.ModelTokenShares.Keys),
                        training.SelectMany(x => x.Features.EffortTokenShares.Keys));
                if (basis is not null && training.Any(x => basis.Score(x) is null)) basis = null;
            }
            QuotaCostObservation[] calibration = ordered.Skip(comparison.Transfer ? 0 : 20).Where(x => x.StartUtc >= training[^1].EndUtc)
                .Take(20).ToArray();
            QuotaCostObservation[] heldout =
                calibration.Length == 20 ? ordered.Where(x => x.StartUtc >= calibration[^1].EndUtc).ToArray() : [];
            QuotaCostObservation[] supported = heldout.Where(x => basis?.Score(x) is not null).ToArray();
            QuotaCostObservation[] unsupported = heldout.Except(supported).ToArray();
            IReadOnlyList<IReadOnlyList<QuotaCostObservation>> generations = ChronologicalEvidence.Blocks(
                supported,
                x => x.StartUtc,
                x => x.EndUtc,
                x => x.ResetUtc);
            bool ready = basis is not null && calibration.Length == 20 && calibration.All(x => basis.Score(x) is not null);
            ...
            if (ready)
            {
                double[] quantities = calibration.Select(x => basis!.Score(x)!.Value).ToArray();
                double curvature = quantities.Average(x => x * x);
                if (curvature > 0)
                {
                    double alpha = 0d;
                    ...
                    {
                        cancellationToken.ThrowIfCancellationRequested();
                        double gradient = quantities.Select((x, j) => x * (x * alpha - Math.Clamp(
                            x * alpha,
                            calibration[j].LowerDelta,
                            calibration[j].UpperDelta))).Average();
                        double next = Math.Max(0, alpha - gradient / curvature);
                    ...
                    scale = alpha;
                    Func<QuotaCostObservation, double> full = QuotaAccountingModel.FitFrozen(
                        calibration,
                        "model-effort",
                        cancellationToken);
                    Func<QuotaCostObservation, double> raw = QuotaAccountingModel.FitFrozen(calibration, "total", cancellationToken);
            ...
            scores.Add(
                new TtEvaluationScore(
                    comparison.Destination.Cohort,
                    comparison.Destination.HorizonHours,
                    basis,
                    basis is null ? "insufficient-unsupported-or-degenerate-basis" :
                    !ready ? "insufficient-or-unsupported-calibration" :
                    scale is null ? "zero-calibration-work" :
                    supported.Length == 0 ? "no-supported-heldout-work" : "research-only-not-promoted",
                    training.Length,
                    calibration.Length,
                    supported.Length,
                    unsupported.Length,
                    unsupported.Sum(x => (double)x.Features.Tokens),
                    QuotaResetGenerationPolicy.Group(supported, x => x.ResetUtc).Count,
                    scale,
                    supported.Length > 0 ? supported.Sum(x => basis!.Score(x)!.Value) : null,
                    scalarLoss,
                    vectorLoss,
                    rawLoss)
                {
                    ScalarMae = scalarMae,
                    FullVectorMae = vectorMae,
                    RawTokenMae = rawMae,
                    ZeroLoss = zeroLoss,
                    ScalarBias = scale is { } a && generations.Count > 0
                        ? generations.Average(g => g.Average(x => a * basis!.Score(x)!.Value - x.ObservedDelta))
                        : null,
                    CumulativeError = scale is { } b && supported.Length > 0
                        ? supported.Sum(x => b * basis!.Score(x)!.Value - x.ObservedDelta)
                        : null,
                    CumulativeErrorLower = scale is { } c && supported.Length > 0
                        ? supported.Sum(x => c * basis!.Score(x)!.Value - x.UpperDelta)
                        : null,
                    CumulativeErrorUpper = scale is { } d && supported.Length > 0
                        ? supported.Sum(x => d * basis!.Score(x)!.Value - x.LowerDelta)
                        : null,
                    BasisCohort = comparison.Source.Cohort,
                    IsTransfer = comparison.Transfer,
                    BasisEndUtc = training[^1].EndUtc,
                    CalibrationEndUtc = calibration.LastOrDefault()?.EndUtc,
                });
        }
        return new TtEvaluation(Version, Methodology, scores);
    }
}