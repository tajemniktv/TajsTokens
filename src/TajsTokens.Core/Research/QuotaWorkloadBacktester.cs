...
public static class QuotaWorkloadBacktester
{
        ...
        string baselineModel = "legacy-ewma")
    {
        ...
        {
            var baseline = QuotaForecastCalibration.Replay(stream.ToArray(), baselineModel, horizonHours, cancellationToken);
            ...
            foreach (var trial in baseline)
            {
                ...
                var training = QuotaWorkloadCorrection.Independent(baseline.Where(x => x.OutcomeUtc <= trial.OriginUtc && x.OriginUtc < trial.OriginUtc && Supported(x)))
                ...
                if (training.Length >= QuotaWorkloadCorrection.MinimumTrainingSamples && Supported(trial))
                {
                    ...
                    double[] Features(QuotaForecastTrial x) => QuotaWorkloadCorrection.Vector(x, origins[x.OriginUtc].RemainingPercent!.Value, featureRows[x.OriginUtc], candidate, models, efforts);
