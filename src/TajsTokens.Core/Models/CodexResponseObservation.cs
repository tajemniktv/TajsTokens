// Taj's Tokens | CodexResponseObservation.cs
...
public sealed record CodexResponseObservation(
    ...
    string? ResponseId,
    CodexResponseSnapshot? Usage,
    CodexResponseSnapshot? TurnUsage,
    CodexResponseSnapshot? ThreadUsage,
    string Diagnostics,
    string ContractVersion = "codex-response/v1");
...
public sealed record CodexResponseEvidencePage(IReadOnlyList<CodexResponseEvidenceRow> Rows, bool Truncated, int CorruptRows)
{
    ...
    public (int Repeated, int Conflicting) CompareActiveCandidates()
    {
        int repeated = 0;
        int conflicts = 0;
        foreach (IGrouping<(string SourceIdentity, string? ReportedThreadId, string? ResponseId), CodexResponseObservation> group in Rows
                     .Where(x => x.IsActive && x.Observation.ResponseId is not null &&
                                 x.Observation.TurnId is not null && x.Observation.RootTurnId is not null &&
                                 x.Observation.RuntimeSessionId is not null &&
                     ...
                     .Select(x => x.Observation).GroupBy(x => (x.SourceIdentity, x.ReportedThreadId, x.ResponseId)))
        {
            ...
            int variants = group.Select(x => (x.TurnId, x.RootTurnId, x.RuntimeSessionId,
