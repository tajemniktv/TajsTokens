// Taj's Tokens | CodexTokenAccounting.cs
...
/// <summary>
///     A pure accounting decision.  SQLite persists the delta and <see cref="NextState" /> in one transaction; it does not repeat any counter
///     inference.
/// </summary>
...
public static class CodexTokenCounterReducer
{
        ...
        bool sourceEventAlreadyPersisted = false)
    {
        ...
        {
            return new CodexTokenAccountingDecision(
                CodexTokenAccountingDecisionKind.Duplicate,
        ...
        {
            return new CodexTokenAccountingDecision(
                CodexTokenAccountingDecisionKind.StaleObservation,
                CodexTokenAccountingDelta.Zero,
                previous,
                false);
        }

        bool hasCompleteLast = HasCompleteLast(observation.LastTokenUsage);
        if (!observation.HasCompleteTotalUsage)
        {
            if (!hasCompleteLast)
            {
                ...
                return new CodexTokenAccountingDecision(
                    CodexTokenAccountingDecisionKind.NoObservation,
                    CodexTokenAccountingDelta.Zero,
                    previous,
                    false);
            }

            Counters observedIncrement = Counters.FromComplete(observation.LastTokenUsage!);
            if (previous is null)
            {
                ...
                return new CodexTokenAccountingDecision(
                    ...
                    ToState(observation, 0, observedIncrement, false),
                    true);
            }
            ...
            Counters synthetic = Counters.From(previous).SaturatingAdd(observedIncrement);
            return new CodexTokenAccountingDecision(
        ...
        Counters current = Counters.From(observation.TotalTokenUsage!);
        if (previous is null)
        {
            Counters firstDelta = SelectDelta(current, null, observation.LastTokenUsage, true);
            return new CodexTokenAccountingDecision(
                CodexTokenAccountingDecisionKind.FirstObservation,
                ToDisjointDelta(firstDelta),
                ToState(observation, 0, current, true),
                true);
        }
        ...
        if (!previous.HasCumulativeBaseline)
        {
            ...
            Counters firstCumulativeDelta = SelectDelta(current, previous, observation.LastTokenUsage, false);
            CodexTokenAccountingDecisionKind firstCumulativeKind = firstCumulativeDelta.IsZero
                ? CodexTokenAccountingDecisionKind.NoIncrement
                : CodexTokenAccountingDecisionKind.MonotonicIncrement;
            return new CodexTokenAccountingDecision(
                firstCumulativeKind,
                ToDisjointDelta(firstCumulativeDelta),
                ToState(observation, previous.Epoch, current, true),
                true);
        }
        ...
        bool regressions = current.HasRegressionFrom(previous);
        if (regressions)
        {
            ...
            Counters resetDelta = SelectDelta(current, previous, observation.LastTokenUsage, true);
            return new CodexTokenAccountingDecision(
                CodexTokenAccountingDecisionKind.CounterReset,
                ToDisjointDelta(resetDelta),
                ToState(observation, checked(previous.Epoch + 1), current, true),
                true);
        }

        Counters delta = SelectDelta(current, previous, observation.LastTokenUsage, false);
        CodexTokenAccountingDecisionKind kind = delta.IsZero
            ? CodexTokenAccountingDecisionKind.NoIncrement
            : CodexTokenAccountingDecisionKind.MonotonicIncrement;
        return new CodexTokenAccountingDecision(kind, ToDisjointDelta(delta), ToState(observation, previous.Epoch, current, true), true);
    }
        ...
        CodexTokenUsageSnapshot? last)
    {
        ...
        Counters increment = Counters.FromComplete(last);
    ...
    {
        return current >= previous || previous - current <= increment;
    }
    ...
    private static bool IsTokscaleStaleRegression(long current, long previous, long last)
    {
        ...
        bool withinTwoPercent = current * 100m >= previous * 98m;
        long doubledLast = SaturatingMultiply(last, 2);
    ...
    {
        return last is not null && last.IsComplete && last.IsNonNegative;
    }
    ...
    private static CodexTokenAccountingDelta ToDisjointDelta(Counters value)
    {
        ...
        long uncachedInput = Math.Max(0, value.InputTokens - value.CachedInputTokens);
        uncachedInput = Math.Max(0, uncachedInput - value.CacheWriteInputTokens);
        long nonReasoningOutput = Math.Max(0, value.OutputTokens - value.ReasoningOutputTokens);
        return new CodexTokenAccountingDelta(
            uncachedInput,
    ...
    {
        return new CodexTokenCounterState(
        ...
        long TotalTokens)
    {
        ...
        {
            return From(observation.TotalTokenUsage);
        }

        public static Counters From(CodexTokenCounterState state)
        {
            return new Counters(
                state.InputTokens,
        ...
        public static Counters From(CodexTokenUsageSnapshot? snapshot)
        {
            return snapshot is null ? Zero : FromSnapshot(snapshot);
        }

        private static Counters FromSnapshot(CodexTokenUsageSnapshot snapshot)
        {
            return new Counters(
                Clamp(snapshot.InputTokens ?? 0),
                Clamp(snapshot.CachedInputTokens ?? 0),
                Clamp(snapshot.CacheWriteInputTokens ?? 0),
                Clamp(snapshot.OutputTokens ?? 0),
                Clamp(snapshot.ReasoningOutputTokens ?? 0),
                Clamp(snapshot.TotalTokens ?? 0));
        }

        public static Counters FromComplete(CodexTokenUsageSnapshot snapshot)
        {
            return new Counters(
                snapshot.InputTokens!.Value,
                snapshot.CachedInputTokens!.Value,
                snapshot.CacheWriteInputTokens!.Value,
                snapshot.OutputTokens!.Value,
                snapshot.ReasoningOutputTokens!.Value,
                snapshot.TotalTokens!.Value);
        }

        public Counters Subtract(CodexTokenCounterState previous)
        {
            return new Counters(
        ...
        public Counters SaturatingAdd(Counters increment)
        {
            return new Counters(
        ...
        {
            return InputTokens < previous.InputTokens ||
