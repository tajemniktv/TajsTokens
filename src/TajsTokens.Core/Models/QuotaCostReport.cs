// Taj's Tokens | QuotaCostReport.cs
...
public sealed record QuotaCostTrial(
...
public sealed record QuotaCostScore(
    ...
    double? BlockMeanIntervalLoss,
    double? ResidualP10,
    double? ResidualMedian,
    double? ResidualP90,
    double? UnexplainedPositiveMovement,
    int BandSamples,
    double? BandIntersectsTargetRate,
    bool MaterialWin,
    string Status,
    IReadOnlyDictionary<string, double> Coefficients,
    IReadOnlyList<QuotaCostTrial> Trials,
...
public sealed record QuotaCostReport(
    string Version,
    string Methodology,
    string Coverage,
    int Observations,
    IReadOnlyDictionary<string, int> QualityCounts,
    IReadOnlyList<QuotaCostScore> Scores)
{
    public DateTimeOffset? DatasetCapturedAtUtc { get; init; }
    public QuotaEvaluationCoverage? EvidenceCoverage { get; init; }
    public IReadOnlyList<QuotaCostConstructionCoverage> ConstructionCoverage { get; init; } = [];
