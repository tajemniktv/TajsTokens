...
public sealed record CodexServerComparisonRow(string Scope, long? ServerTokens, long? LocalTokens,
    long? EstimatedCreditsMicros, AccountEvidenceClass Association, string Detail)
{
    public double? LocalToServerRatio => ServerTokens is > 0 && LocalTokens is not null ? (double)LocalTokens / ServerTokens : null;
}
public sealed record CodexServerComparisonReport(DateTimeOffset ReadAtUtc,
    IReadOnlyList<CodexServerObservation> LatestObservations, IReadOnlyList<CodexServerComparisonRow> Comparisons,
    int UserDeclaredAssociations, string Limitations)
{
