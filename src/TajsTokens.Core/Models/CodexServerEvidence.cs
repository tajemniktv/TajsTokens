// Taj's Tokens | CodexServerEvidence.cs
...
public enum ServerEvidenceState
{
    Available,
    Empty,
    Unavailable,
...
public enum AccountEvidenceClass
{
    ProviderVerified,
    ServerCorrelated,
    UserDeclaredSingleAccount,
    Unattributed,
    Conflicting,
}

public enum CodexServerSurface
{
    AccountActivity,
    ThreadUsage,
    PlanHistory,
...
public sealed record CodexAccountActivity(
    long? LifetimeTokens,
    long? PeakDailyTokens,
    long? LongestRunningTurnSec,
...
public sealed record CodexThreadUsage(
    string ThreadId,
    long EstimatedUsageCreditsMicros,
    long? EstimatedUsageUsdMicros,
    IReadOnlyList<CodexThreadUsageGroup> Groups);

public sealed record CodexThreadUsageGroup(
    string? Model,
...
public sealed record CodexServerAccountBracket(
    string? BeforeAccountKey,
    DateTimeOffset? BeforeCollectedAtUtc,
    string? AfterAccountKey,
    DateTimeOffset? AfterCollectedAtUtc);

/// <summary>Only allowlisted, content-free fields are serializable here. No raw response or credentials.</summary>
public sealed record CodexServerObservation(
    string Id,
    CodexServerSurface Surface,
    string? ThreadId,
    DateTimeOffset FetchStartedAtUtc,
    DateTimeOffset CollectedAtUtc,
    string ContractVersion,
    string? ClientVersion,
    ServerEvidenceState State,
    string Detail)
{
...
public sealed record CodexDailyReport(
    ...
    string? PolicyBefore,
    string? PolicyAfter,
    IReadOnlyList<CodexDailyReportRow> Days);

public sealed record CodexDailyReportRow(
    string Date,
    decimal? Credits,
    decimal? OnDemandCredits,
