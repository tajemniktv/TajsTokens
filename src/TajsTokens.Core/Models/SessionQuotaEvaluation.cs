// Taj's Tokens | SessionQuotaEvaluation.cs
// Copyright (C) 2026 - 2026 Grzegorz Kaczmarski (TajemnikTV)
// All Rights Reserved.

namespace TajsTokens.Core.Models;

public sealed record SessionQuotaTrial(
    DateTimeOffset OriginUtc,
    DateTimeOffset EndUtc,
    DateTimeOffset ResetUtc,
    bool RecordedActivity,
    double ActivityProbability,
    double ConditionalQuota,
    double ExpectedQuota,
    double ObservedQuota,
    double ConditionalError,
    double ExpectedError,
    double PaceError,
    double ExpectedIntervalLoss,
    double PaceIntervalLoss,
    double? LowerExpectedQuota,
    double? UpperExpectedQuota)
{
    public double? IncumbentError { get; init; }
    public double? IncumbentIntervalLoss { get; init; }
}

public sealed record SessionQuotaScore(
    QuotaHistoryCohort Cohort,
    double HorizonHours,
    int TrainingIntervals,
    int WithheldIntervals,
    IReadOnlyList<SessionQuotaTrial> Trials,
    int ResetGenerations,
    int ActiveOutcomes,
    double? ConditionalMae,
    double? ConditionalPaceMae,
    double? ExpectedMae,
    double? PaceMae,
    double? IntervalLoss,
    double? PaceIntervalLoss,
    int BandOrigins,
    double? ReportedBandCoverage,
