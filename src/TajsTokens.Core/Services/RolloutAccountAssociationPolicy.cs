// Taj's Tokens | RolloutAccountAssociationPolicy.cs
...
{
    public static IReadOnlyList<RolloutAccountAssociation> Matches(
        string provider,
        string profile,
        ...
        DateTimeOffset asOf)
    {
        return associations.Where(x => x.IsValid && x.AssertedAtUtc <= asOf && x.Provider == provider &&
    ...
    public static RolloutAccountAssociation? Resolve(
        QuotaSnapshot row,
        IReadOnlyList<RolloutAccountAssociation> associations,
        DateTimeOffset asOf)
    {
        if (row.AccountKey is not null || row.Authority != QuotaObservationAuthority.EmbeddedObservation) return null;
        IReadOnlyList<RolloutAccountAssociation> matches = Matches(
            row.Provider,
            row.Profile,
            row.SourceIdentity,
            row.SessionId,
        ...
        return matches.Select(x => x.AccountKey).Distinct().Count() == 1
            ? matches.OrderBy(x => x.AssertedAtUtc).First()
            : null;
    }

    public static IReadOnlyList<RolloutAccountAssociation> Create(
        ...
        DateTimeOffset assertedAt)
    {
        if (!data.Quota.Any(x => x.AccountKey == accountKey && x.Authority == QuotaObservationAuthority.ProviderAuthoritative))
            throw new ArgumentException(
                "Select a recorded provider account; an arbitrary account identity cannot be asserted.",
                nameof(accountKey));
        return data.Quota.Where(x => x.AccountKey is null && x.Authority == QuotaObservationAuthority.EmbeddedObservation &&
            ...
            .Select(g => new RolloutAccountAssociation(
                Guid.NewGuid().ToString("N"),
                g.Key.Provider,
                g.Key.Profile,
                g.Key.SourceIdentity!,
                g.Key.SessionId!,
                g.Min(x => x.CapturedAtUtc),
