...
public static partial class QuotaPredictionService
{
    ...
    public static ScenarioEstimate Simulate(ScenarioRequest request, IReadOnlyList<ScenarioHistorySample> history,
        DateTimeOffset? evaluatedAtUtc = null)
    {
        ...
        ScenarioWindowEstimate Build(QuotaWindowKind kind)
        {
            ...
            if (samples.Length < MinimumScenarioSamples)
                return Unknown($"Not enough matching history: need {MinimumScenarioSamples} intervals; have {samples.Length}. Requested model/effort cohorts are never replaced silently.", samples.Length);
            ...
            var errors = ChronologicalEvidence.Blocks(trials.Where(x => x.Sample.ResetUtc is not null &&
                    Hours(x.Sample) >= request.DurationHours / 2 && Hours(x.Sample) <= request.DurationHours * 2),
                    x => x.Sample.StartUtc, x => x.Sample.EndUtc, x => x.Sample.ResetUtc!.Value)
            ...
            return new ScenarioWindowEstimate(kind, true, samples.Length, prediction,
                ...
                (radius is null ? "Uncertainty is learning: fewer than 8 comparable held-out chronological blocks. " : "Empirical 80%-target band from earlier held-out chronological blocks; coverage can change with workload. ") +
