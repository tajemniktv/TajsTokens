// Taj's Tokens | ComposedQuotaBreakdowns.cs
...
public static class ComposedQuotaBreakdowns
{
    ...
    public static IReadOnlyList<ComposedQuotaBreakdown> Build(IReadOnlyList<ComposedQuotaTrial> trials)
    {
        ...
        {
            ComposedQuotaTrial[] items = rows.ToArray();
            ComposedQuotaTrial[] incumbent = items.Where(x => x.IncumbentIntervalLoss is not null).ToArray();
            ComposedQuotaTrial[] zero = items.Where(x => x.ZeroUseIntervalLoss is not null).ToArray();
            ComposedQuotaTrial[] bands = items.Where(x =>
                    x.ObservedRemainingPercent is not null && x.LowerRemainingPercent is not null && x.UpperRemainingPercent is not null)
                .ToArray();
            double[] misses = items.Where(x => HasMeterEnvelope(x) && double.IsFinite(x.PredictedDelta))
                .Select(x => Math.Max(0, x.LowerObservedDelta!.Value - x.PredictedDelta)).ToArray();
            result.Add(
                new ComposedQuotaBreakdown(
                    dimension,
                    label,
                    items.Length,
                    QuotaResetGenerationPolicy.Group(items, x => x.ResetUtc).Count,
                    items.Average(x => x.IntervalLoss),
                    items.Average(x => x.CostOnlyIntervalLoss),
                    items.Average(x => x.PaceIntervalLoss),
                    items.Average(x => x.PredictedDelta - x.ObservedDelta),
                    incumbent.Length,
                    Mean(incumbent.Select(x => x.IntervalLoss)),
                    Mean(incumbent.Select(x => x.IncumbentIntervalLoss!.Value)),
                    zero.Length,
                    Mean(zero.Select(x => x.IntervalLoss)),
                    Mean(zero.Select(x => x.ZeroUseIntervalLoss!.Value)),
                    bands.Length,
                    Mean(
                        bands.Select(x =>
                            x.ObservedRemainingPercent >= x.LowerRemainingPercent && x.ObservedRemainingPercent <= x.UpperRemainingPercent
                                ? 1d
                                : 0d)),
                    Mean(bands.Select(x => x.UpperRemainingPercent!.Value - x.LowerRemainingPercent!.Value)))
                {
                    MeteredOutcomes = misses.Length, UnderpredictedOutcomes = misses.Count(x => x > 0), MeanUnderprediction = Mean(misses),
                });
        }

        foreach (IReadOnlyList<ComposedQuotaTrial> reset in QuotaResetGenerationPolicy.Group(trials, x => x.ResetUtc))
            Add("reset", reset.Min(x => x.ResetUtc).ToString("O"), reset);

        void Partition(string dimension, Func<ComposedQuotaTrial, string> key)
        {
            foreach (IGrouping<string, ComposedQuotaTrial> group in trials.GroupBy(key).OrderBy(x => x.Key, StringComparer.Ordinal))
                Add(dimension, group.Key, group);
        }
        ...
        Partition(
            "recorded-outcome",
            x => x.RecordedOutcomeTokens is null ? "unknown" : x.RecordedOutcomeTokens > 0 ? "positive-recorded-work" : "no-recorded-work");
        Partition(
            "outcome-quota-movement",
            x => !HasMeterEnvelope(x) ? "unknown-meter-envelope" :
                x.LowerObservedDelta >= 5 ? "at-least-5pp" :
                x.UpperObservedDelta < 5 ? "below-5pp" : "straddles-5pp");
        Partition(
            "outcome-category-coverage",
            x => x.CompleteOutcomeTokenCategories switch
            {
                true => "complete-reported-vectors", false => "incomplete-reported-vectors", _ => "unknown",
            });
        Partition(
            "outcome-quality",
            x => x.OutcomeQualityFlags.Count == 0
                ? "no-recorded-flags-not-proven-complete"
    ...
    {
        return trial.LowerObservedDelta is double low && double.IsFinite(low) && low >= 0 &&
    ...
    private static string Mix(IReadOnlyDictionary<string, double> shares)
    {
        ...
        KeyValuePair<string, double> largest = shares.OrderByDescending(x => x.Value).ThenBy(x => x.Key, StringComparer.Ordinal).First();
