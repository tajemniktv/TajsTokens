// Taj's Tokens | ComposedQuotaPolicy.cs
...
{
    public static IReadOnlyList<QuotaHorizonPrediction> Apply(
        CodexForecastDataset data,
        QuotaSnapshot anchor,
        IReadOnlyList<QuotaHorizonPrediction> incumbent,
        CancellationToken cancellationToken = default)
    {
        ...
        CodexForecastDataset prefix = data with
        {
            ...
            Quota = data.Quota.Where(x => (QuotaHistoryPolicy.Cohort(x) == QuotaHistoryPolicy.Cohort(anchor) ||
                                           RolloutAccountAssociationPolicy.Resolve(x, data.AccountAssociations, anchor.CapturedAtUtc)
                                               ?.AccountKey == anchor.AccountKey) &&
                                          x.CapturedAtUtc <= anchor.CapturedAtUtc &&
                                          (x.CollectedAtUtc is null || x.CollectedAtUtc <= anchor.CapturedAtUtc)).ToArray(),
            Tokens = data.Tokens.Where(x => x.ObservedAtUtc <= anchor.CapturedAtUtc && x.CapturedAtUtc <= anchor.CapturedAtUtc).ToArray(),
            Workload =
            data.Workload.Where(x => x.ObservedAtUtc <= anchor.CapturedAtUtc && x.CapturedAtUtc <= anchor.CapturedAtUtc).ToArray(),
            Context = data.Context.Where(x => x.ObservedAtUtc <= anchor.CapturedAtUtc && x.CapturedAtUtc <= anchor.CapturedAtUtc).ToArray(),
        };
        QuotaCostReport cost = QuotaAccountingModel.Evaluate(prefix, cancellationToken);
        QuotaCostScore[] eligible = cost.Scores.Where(x =>
            x.Cohort == QuotaHistoryPolicy.Cohort(anchor) && x.MaterialWin && x.CandidateShiftTimes.Count == 0 &&
        ...
        ComposedQuotaEvaluation evaluation = ComposedQuotaValidation.Evaluate(prefix, cancellationToken, horizons: [.5, 2d]);
        ComposedQuotaEvaluation strict = ComposedQuotaValidation.Evaluate(
            prefix,
            cancellationToken,
            ForecastReplayAvailability.CollectedByOrigin,
            [.5, 2d]);
        IReadOnlyList<QuotaCostObservation> observations = QuotaCostObservationBuilder.Build(prefix, cancellationToken);
        return incumbent.Select(baseline =>
        {
            ComposedQuotaScore[] candidates = evaluation.Scores.Where(score =>
                    score.Cohort == QuotaHistoryPolicy.Cohort(anchor) && score.HorizonHours == baseline.HorizonHours &&
                    ...
                    SupportsSelection(score, anchor.CapturedAtUtc) && strict.Scores.Any(s => s.Cohort == score.Cohort &&
                        s.HorizonHours == score.HorizonHours && s.CostModel == score.CostModel &&
                        SupportsStrictSelection(s, anchor.CapturedAtUtc)))
                .OrderBy(x => x.IntervalLoss).ToArray();
            if (candidates.Length == 0) return baseline;
            ComposedQuotaScore selected = candidates[0];
            ComposedQuotaScore liveEvidence = strict.Scores.Single(x =>
                x.Cohort == selected.Cohort && x.HorizonHours == selected.HorizonHours && x.CostModel == selected.CostModel);
            ComposedQuotaScore? total = evaluation.Scores.FirstOrDefault(x =>
                x.Cohort == selected.Cohort && x.HorizonHours == selected.HorizonHours && x.CostModel == "total");
            ...
            if (selected.CostModel != "total" && (total?.IntervalLoss is not { } totalLoss ||
                                                  selected.IntervalLoss + 0.1 >= totalLoss ||
                                                  selected.IntervalLoss >= totalLoss * 0.9)) return baseline;
            PredictedWorkload? workload = TokenWorkloadPredictionService.PredictHorizon(
                prefix,
                anchor.CapturedAtUtc,
                baseline.HorizonHours,
                ForecastReplayAvailability.CollectedByOrigin,
                cancellationToken)?.Composition;
            if (workload is null) return baseline;
            QuotaCostObservation[] training = QuotaCostTrainingPolicy.Select(
                observations,
                selected.Cohort,
                baseline.HorizonHours,
                selected.AssertedTrainingIntervals > 0);
            if (training.Length < 20 || !QuotaAccountingModel.HasTrainingWork(training)) return baseline;
            ...
            CodexNumericInference inference = QuotaAccountingModel.CaptureFrozenInference(
                training,
                selected.CostModel.Replace("asserted-", "", StringComparison.Ordinal),
                ComposedQuotaValidation.Project(training[0], workload),
                anchor.RemainingPercent!.Value,
                cancellationToken);
            (double? Radius, int Blocks) calibration = CalibrateUncertainty(liveEvidence.Trials, anchor.CapturedAtUtc);
            if (calibration.Radius is not { } radius) return baseline;
            double remaining = inference.Reconstruct();
            return baseline with
            {
                RemainingPercent = remaining,
                ExpectedUsagePercent = anchor.RemainingPercent.Value - remaining,
                Inference = inference,
                Model = ComposedQuotaValidation.Version + "/" + selected.CostModel,
                UsesWorkload = true,
                TrainingSamples = training.Length,
                ValidationSamples = selected.HeldOutIntervals,
                ...
                IntervalSamples = calibration.Blocks,
                Explanation =
                "Origin-only predicted workload composition through frozen account/cohort-local cost weights earned selection over pace and incumbent. " +
                ...
                "Empirical 80%-target range from completed-block maximum errors; not an exhaustion probability or guarantee. " +
    ...
    public static (double? Radius, int Blocks) CalibrateUncertainty(
        IEnumerable<ComposedQuotaTrial> trials,
    ...
    {
        return AssessSelection(score, origin).Passed;
    }
    ...
    public static ComposedSelectionAssessment AssessSelection(ComposedQuotaScore score, DateTimeOffset origin, bool requireStrict = false)
    {
        ...
        IReadOnlyList<IReadOnlyList<ComposedQuotaTrial>> generations = ChronologicalEvidence.Blocks(
            score.Trials,
            x => x.OriginUtc,
            x => x.OutcomeUtc,
            x => x.ResetUtc);
        int pairs = score.Trials.Count(x => x.IncumbentIntervalLoss is not null);
        if (score.HeldOutIntervals < 16 || score.Trials.Count < 16) reasons.Add("At least 16 held-out outcomes are required.");
        if (generations.Sum(x => x.Count) < 16)
            reasons.Add("At least 16 non-overlapping valid outcomes are required; duplicate polls do not add support.");
        if (generations.Count < 8) reasons.Add("At least eight chronological evaluation blocks are required.");
        ...
        double? incumbentLoss = generations.Count == 0 || pairs != score.Trials.Count
            ? null
            : generations.Average(g => g.Average(x => x.IncumbentIntervalLoss!.Value));
        if (loss is { } l && paceLoss is { } p && !(l + .1 < p && l < p * .9))
            reasons.Add("The block-balanced improvement over pace is not both greater than 0.1pp and 10%.");
        if (loss is { } il && incumbentLoss is { } b)
        {
            if (!(il + .1 < b && il < b * .9))
                reasons.Add("The block-balanced improvement over incumbent is not both greater than 0.1pp and 10%.");
            if (generations.Count(g => g.Average(x => x.IntervalLoss) < g.Average(x => x.IncumbentIntervalLoss!.Value)) <=
                generations.Count / 2)
                reasons.Add("The challenger does not beat incumbent in a majority of chronological blocks.");
        }
        if (requireStrict)
        {
            if (score.Availability != ForecastReplayAvailability.CollectedByOrigin || score.Trials.Any(x =>
                    x.Availability != ForecastReplayAvailability.CollectedByOrigin ||
                    x.Workload.Availability != ForecastReplayAvailability.CollectedByOrigin))
            ...
            if (incumbentLoss is not null && !generations.OrderByDescending(g => g.Max(x => x.OutcomeUtc)).Take(2)
                ...
                reasons.Add("One of the latest two blocks loses to pace or incumbent.");
            if (score.AssertedTrainingIntervals > 0 && (score.HeldOutIntervals < 32 || generations.Count < 12))
                reasons.Add("User-asserted training requires at least 32 strict outcomes across 12 chronological blocks.");
        }
        return new ComposedSelectionAssessment(
            reasons.Count == 0,
            requireStrict,
            score.Trials.Count,
            generations.Count,
    ...
    {
        return AssessSelection(score, origin, true).Passed;
    }
    ...
    private static bool SupportsAssertedCost(ComposedQuotaScore score, IReadOnlyList<ComposedQuotaScore> all)
    {
        ...
        ComposedQuotaScore? local =
            all.FirstOrDefault(x => x.Cohort == score.Cohort && x.HorizonHours == score.HorizonHours && x.CostModel == "total");
        if (local is null || score.Trials.Count < 16) return false;
        Dictionary<DateTimeOffset, ComposedQuotaTrial> lookup = local.Trials.ToDictionary(x => x.OriginUtc);
        ...
        IReadOnlyList<IReadOnlyList<ComposedQuotaTrial>> generations = ChronologicalEvidence.Blocks(
            ...
            x => x.OutcomeUtc,
            x => x.ResetUtc);
        double loss = generations.Average(g => g.Average(x => x.CostOnlyIntervalLoss));
        double baseline = generations.Average(g => g.Average(x => lookup[x.OriginUtc].CostOnlyIntervalLoss));
