...
public static class RecentScenarioPatternBuilder
{
    ...
    {
        RecentScenarioPattern Unknown(string reason) => new(now, false, 0, 0, null, null, reason);
        if (data.CapturedAtUtc < now.AddMinutes(-10)) return Unknown("Telemetry snapshot is stale; refresh before using a recent pattern.");
        var latest = data.Tokens.Where(x => x.ObservedAtUtc <= now && x.CapturedAtUtc <= now && x.ReportedTotalTokens > 0)
            .MaxBy(x => x.ObservedAtUtc);
        if (latest is null || latest.ObservedAtUtc < now.AddMinutes(-10))
            return Unknown("No recent positive local token activity. This does not establish idleness or exclude activity elsewhere; enter a hypothetical workload manually.");
        var features = CodexForecastFeatureBuilder.Build(data, now, availability: ForecastReplayAvailability.CollectedByOrigin);
        ...
        string? Exclusive(IReadOnlyDictionary<string, double> shares) =>
            shares.Count == 1 && shares.First().Value >= .999 ? shares.First().Key : null;
        var model = Exclusive(features.ModelTokenShares);
        var effort = Exclusive(features.EffortTokenShares);
        return new(now, true, features.TokenActiveRootSessions, features.TokenActiveSubagentSessions, model, effort,
