...
{
    public const string Version = "tt-lab/v1";
    public const string Methodology = "Experimental local index, reconstructed completed-work cost evaluation, not a forecast. " +
        ...
        "This within-cohort experiment does not establish cross-regime or cross-user comparability. Basis is a reproducible research result, " +
    ...
    public static TtEvaluation Evaluate(IReadOnlyList<QuotaCostObservation> rows, CancellationToken cancellationToken = default)
    {
        ...
        foreach (var group in rows.Where(x => x.Cohort.AccountKey is not null).GroupBy(x => (x.Cohort, x.HorizonHours)))
        {
            ...
            var calibration = ordered.Skip(20).Where(x => x.StartUtc >= training[^1].EndUtc).Take(20).ToArray();
            ...
            scores.Add(new(group.Key.Cohort, group.Key.HorizonHours, basis,
