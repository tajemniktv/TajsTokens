...
{
    public static QuotaCostObservation[] Select(IReadOnlyList<QuotaCostObservation> rows,
        QuotaHistoryCohort cohort, double horizon, bool useAssertions)
    {
        var native = rows.Where(x => x.Cohort == cohort && x.HorizonHours == horizon).OrderBy(x => x.StartUtc).Take(20).ToArray();
        if (!useAssertions || native.Length < 20 || cohort.AccountKey is null || cohort.PlanType is null || cohort.LimitId is null) return native;
        var selected = new List<QuotaCostObservation>();
        DateTimeOffset? end = null;
        foreach (var row in rows.Where(x => x.Attribution == QuotaAccountAttribution.UserAsserted &&
                     ...
                     x.Cohort.Kind == cohort.Kind && x.Cohort.WindowMinutes == cohort.WindowMinutes &&
