// Taj's Tokens | QuotaCostObservationBuilder.cs
...
public static class QuotaCostObservationBuilder
{
    ...
    public const string CoverageBoundary =
        "Construction counts eligible-epoch interior start candidates, not raw quota readings or independent outcomes. " +
    ...
    public static IReadOnlyList<QuotaCostObservation> Build(
        CodexForecastDataset data,
        CancellationToken cancellationToken = default,
        bool userConfirmedRolloutOwnership = false,
        IReadOnlyList<double>? horizons = null)
    {
        return BuildDetailed(data, cancellationToken, userConfirmedRolloutOwnership, horizons).Observations;
    }

    public static QuotaCostObservationBuild BuildDetailed(
        CodexForecastDataset data,
        CancellationToken cancellationToken = default,
        bool userConfirmedRolloutOwnership = false,
        IReadOnlyList<double>? horizons = null)
    {
        ...
        foreach (IReadOnlyList<QuotaSnapshot> epoch in QuotaForecastCalibration.SplitEpochs(QuotaHistoryPolicy.ReplayRows(stream)))
        foreach (double horizon in horizons ?? [0.5, 2d])
        {
            ...
            for (int i = 1; i < epoch.Count - 1; i++)
            {
                ...
                QuotaSnapshot start = epoch[i];
                candidates++;
                if (start.CapturedAtUtc < previousEnd)
                {
                    Reject("overlaps-selected-interval");
                    continue;
                }
                if (start.UsedPercent >= 100)
                {
                    Reject("saturated-origin");
                    continue;
                }
                if (start.CapturedAtUtc - epoch[0].CapturedAtUtc < TimeSpan.FromMinutes(15))
                {
                    Reject("epoch-warmup");
                    continue;
                }
                DateTimeOffset target = start.CapturedAtUtc.AddHours(horizon);
                QuotaSnapshot? end = epoch.Skip(i + 1).FirstOrDefault(x => x.CapturedAtUtc >= target);
                if (end is null)
                {
                    Reject("no-outcome-in-epoch");
                    continue;
                }
                if (end.CapturedAtUtc - target > TimeSpan.FromMinutes(Math.Min(5, horizon * 60 * .2)))
                {
                    Reject("outcome-beyond-poll-tolerance");
                    continue;
                }
                ...
                if (data.Quota.Any(x => x.CapturedAtUtc > start.CapturedAtUtc && x.CapturedAtUtc <= end.CapturedAtUtc &&
                                        ...
                                        QuotaHistoryPolicy.Cohort(x) != stream.Key))
                {
                    Reject("intervening-incompatible-cohort");
                    continue;
                }
                // Saturation hides any further usage; it is not an ordinary bounded delta target.
                if (end.UsedPercent >= 100)
                {
                    Reject("saturated-outcome");
                    continue;
                }
                previousEnd = end.CapturedAtUtc;
                double hours = (end.CapturedAtUtc - start.CapturedAtUtc).TotalHours;
                CodexForecastFeatures features = CodexForecastFeatureBuilder.Build(data, end.CapturedAtUtc, hours);
                CodexPredictiveTokenEvent[] tokens = data.Tokens
                    .Where(x => x.ObservedAtUtc > start.CapturedAtUtc && x.ObservedAtUtc <= end.CapturedAtUtc).ToArray();

                double Sum(Func<CodexPredictiveTokenEvent, long> field)
                {
                    return tokens.Sum(x => (double)Math.Max(0, field(x)));
                }

                double[] categories =
                [
                    Sum(x => x.UncachedInputTokens), Sum(x => x.CacheReadTokens),
                    Sum(x => x.CacheWriteTokens), Sum(x => x.NonReasoningOutputTokens), Sum(x => x.ReasoningOutputTokens),
                ];
                CodexWorkloadObservation[] runtime = data.Workload.Where(x => x.ObservedAtUtc > start.CapturedAtUtc &&
                                                                              x.ObservedAtUtc <= end.CapturedAtUtc &&
                                                                              x.EventType == "task_complete" &&
                                                                              x.TimeToFirstTokenMilliseconds >= 0).ToArray();
                ...
                if (stream.Key.AccountKey is null)
                    flags.Add(
                        userConfirmedRolloutOwnership && start.Authority == QuotaObservationAuthority.EmbeddedObservation
                            ? "user-confirmed-rollout-ownership-native-account-id-absent"
                            : "native-account-id-absent");
                ...
                if (tokens.Any(x => x.CapturedAtUtc is null || x.CapturedAtUtc > end.CapturedAtUtc))
                    flags.Add("backfilled-or-unknown-collection-time");
                ...
                bool rounded = start.Authority == QuotaObservationAuthority.ProviderAuthoritative &&
                               start.UsedPercent == Math.Truncate(start.UsedPercent!.Value) &&
                               end.UsedPercent == Math.Truncate(end.UsedPercent!.Value);
                double halfWidth = rounded ? 0.5 : 1;
                if (!rounded) flags.Add("meter-precision-unverified-sensitivity-only");
                double lower = Math.Max(
                    0,
                    Math.Max(0, end.UsedPercent!.Value - halfWidth) - Math.Min(100, start.UsedPercent!.Value + halfWidth));
                double upper = Math.Max(
                    0,
                    Math.Min(100, end.UsedPercent.Value + halfWidth) - Math.Max(0, start.UsedPercent.Value - halfWidth));
                QuotaSnapshot[] prefix = epoch.Take(i + 1).ToArray();
                RolloutAccountAssociation? association = RolloutAccountAssociationPolicy.Resolve(
                    start,
                    data.AccountAssociations,
                    data.CapturedAtUtc);
                if (association is not null && association.Id !=
                    RolloutAccountAssociationPolicy.Resolve(end, data.AccountAssociations, data.CapturedAtUtc)?.Id)
                    association = null;
                ...
                if (association is not null) tokenCollected = tokenCollected.Append(association.AssertedAtUtc).ToArray();
                DateTimeOffset?[] collected = tokenCollected
                    ...
                    .Concat(
                        data.Context.Where(x => x.ObservedAtUtc > start.CapturedAtUtc && x.ObservedAtUtc <= end.CapturedAtUtc)
                            .Select(x => x.CapturedAtUtc)).ToArray();
                result.Add(
                    new QuotaCostObservation(
                        stream.Key,
                        epoch[0].CapturedAtUtc,
                        epoch[0].ResetsAtUtc!.Value,
                        start.CapturedAtUtc,
                        end.CapturedAtUtc,
                        horizon,
                        start.UsedPercent.Value,
                        end.UsedPercent.Value,
                        lower,
                        upper,
                        rounded ? "app-server-rounding-envelope" : "unknown-precision-1pp-endpoint-sensitivity",
                        start.RemainingPercent!.Value - QuotaPaceModels.ProjectRemaining(prefix, "legacy-ewma", hours),
                        categories,
                        features,
                        runtime.Length > 0 ? runtime.Average(x => (double)x.TimeToFirstTokenMilliseconds!.Value) : null,
                        runtime.Length,
                        flags)
                    {
            ...
            coverage.Add(new QuotaCostConstructionCoverage(stream.Key, horizon, candidates, result.Count - initialCount, rejected));
        }
        return new QuotaCostObservationBuild(
            result,
            coverage.GroupBy(x => (x.Cohort, x.HorizonHours)).Select(g =>
                new QuotaCostConstructionCoverage(
                    g.Key.Cohort,
                    g.Key.HorizonHours,
                    g.Sum(x => x.CandidateStarts),
                    g.Sum(x => x.BuiltIntervals),
                    g.SelectMany(x => x.RejectedStarts).GroupBy(x => x.Key, StringComparer.Ordinal)
                        .OrderBy(x => x.Key, StringComparer.Ordinal)
                        .ToDictionary(x => x.Key, x => x.Sum(y => y.Value), StringComparer.Ordinal))).ToArray());
    }
}