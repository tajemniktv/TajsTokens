...
public enum NowcastActivityState { RecentActivity, QuietOpenTurn, NoRecentActivity, Unknown }

public sealed record NowcastActivity(NowcastActivityState State, DateTimeOffset? LastActivityAtUtc,
    int ObservedOpenTurns, string Explanation)
{
...
public static class CodexNowcastActivity
{
    ...
    internal static bool IsActivityEvent(string type) => type is "task_started" or "function_call" or
        "function_call_output" or "custom_tool_call" or "custom_tool_call_output";

    public static NowcastActivity Evaluate(CodexForecastDataset data, DateTimeOffset origin,
        ForecastReplayAvailability availability = ForecastReplayAvailability.ReconstructedEventTime)
    {
        bool Available(DateTimeOffset? at) => availability != ForecastReplayAvailability.CollectedByOrigin || at <= origin;
        var tokens = data.Tokens.Where(x => x.ObservedAtUtc <= origin && Available(x.CapturedAtUtc) && x.ReportedTotalTokens > 0);
        var workload = data.Workload.Where(x => x.ObservedAtUtc <= origin && Available(x.CapturedAtUtc)).ToArray();
        // Settings, session metadata and context snapshots alone do not prove ongoing work.
        var activity = workload.Where(x => IsActivityEvent(x.EventType));
        var latest = tokens.Select(x => (DateTimeOffset?)x.ObservedAtUtc)
            .Concat(activity.Select(x => x.ObservedAtUtc)).DefaultIfEmpty().Max();
        var open = workload.Where(x => x.TurnId is not null && x.EventType is "task_started" or "task_complete" or "turn_aborted")
        ...
        if (latest is { } at && origin - at < IdleAfter)
            return new(NowcastActivityState.RecentActivity, at, open,
                "Recent local token/turn/tool activity; continued activity is uncertain. No activity probability is estimated.");
        if (open > 0)
            return new(NowcastActivityState.QuietOpenTurn, latest, open,
        ...
        if (latest is not null)
            return new(NowcastActivityState.NoRecentActivity, latest, 0,
        ...
        return new(NowcastActivityState.Unknown, null, 0,
