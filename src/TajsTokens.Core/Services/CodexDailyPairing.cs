...
public sealed record CodexDailyPair(string Date, decimal? NativeCredits, decimal? RelativePercent,
    decimal? CreditsPerPercentagePoint, IReadOnlyList<string> Exclusions);

public sealed record CodexDailyPairingReport(string Policy, string? CountsObservationId,
...
public static class CodexDailyPairing
{
    ...
    public static CodexDailyPairingReport Evaluate(IEnumerable<CodexServerObservation> observations)
    {
        ...
        var snapshots = observations.ToArray();
        var counts = Latest(CodexServerSurface.DailyCounts);
        var relative = Latest(CodexServerSurface.DailyRelativeUsage);
        CodexServerObservation? Latest(CodexServerSurface surface) => snapshots.Where(x => x.Surface == surface)
        ...
        if (a is not null && b is not null)
        {
            ...
            else if (a.DataFreshness is null) assumptions.Add("no backend as-of supplied; same bracketed fetch is used as provisional revision compatibility");
            if (a.GroupBy is null || b.GroupBy is null) assumptions.Add("day grain follows the adapter request; response grain was not supplied");
        ...
        {
            var countDays = a.Days.ToDictionary(x => x.Date);
            var relativeDays = b.Days.ToDictionary(x => x.Date);
            var completedBefore = DateOnly.FromDateTime((counts!.FetchStartedAtUtc < relative!.FetchStartedAtUtc ? counts.FetchStartedAtUtc : relative.FetchStartedAtUtc).UtcDateTime);
            foreach (var date in countDays.Keys.Union(relativeDays.Keys).Order(StringComparer.Ordinal))
            {
                var reasons = new List<string>();
                countDays.TryGetValue(date, out var count);
                relativeDays.TryGetValue(date, out var usage);
                if (count is null || usage is null) reasons.Add("date-not-in-both-reports");
                if (!DateOnly.TryParseExact(date, "yyyy-MM-dd", CultureInfo.InvariantCulture, DateTimeStyles.None, out var day)) reasons.Add("invalid-date");
                ...
                if (usage?.SurfaceUsage is not { Count: > 0 } surfaces) reasons.Add("missing-relative-surfaces");
                else if (surfaces.Values.Any(x => x < 0)) reasons.Add("negative-relative-usage");
                else
                {
                    try { percent = surfaces.Values.Sum(); }
                    catch (OverflowException) { reasons.Add("relative-sum-overflow"); }
                    if (percent is <= MinimumPercent) reasons.Add("zero-or-tiny-relative-usage");
                }
                decimal? ratio = null;
                if (exclusions.Count == 0 && reasons.Count == 0)
                {
                    try { ratio = count!.Credits!.Value / percent!.Value; }
                    catch (OverflowException) { reasons.Add("ratio-overflow"); }
                }
    ...
    private static bool Scoped(CodexServerObservation row) => row.AccountEvidence == AccountEvidenceClass.ServerCorrelated &&
