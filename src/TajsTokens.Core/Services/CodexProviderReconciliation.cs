// Taj's Tokens | CodexProviderReconciliation.cs
...
public sealed record CodexPeriodReconciliation(
    string PeriodId,
    ...
    int Captures,
    int Revisions,
    long AttributedLocalTokens,
    double? LastCompatibleMeterPercent,
...
public static class CodexProviderReconciliation
{
    ...
    private static IEnumerable<CodexServerObservation> LatestAttempts(
        IEnumerable<CodexServerObservation> observations,
        CodexServerSurface surface)
    {
        CodexServerObservation[] attempts = observations
            .Where(x => x.Surface == surface && x.ClientVersion == "codex-historical-analytics/v1")
        ...
        DateTimeOffset? barrier = attempts.FirstOrDefault(x => x.CorrelatedAccountKey is null &&
                                                               x.State is not (ServerEvidenceState.Available or ServerEvidenceState.Empty))
            ?.CollectedAtUtc;
        return attempts.Where(x => barrier is null || x.CollectedAtUtc > barrier)
            .GroupBy(x => x.CorrelatedAccountKey).Select(g => g.First());
    }

    public static IReadOnlyList<CodexChatAccounting> Tasks(
        IEnumerable<CodexServerObservation> observations,
        IReadOnlyList<CodexLedgerEntry> ledger,
        CodexSelection selection)
    {
        ...
        return LatestAttempts(observations, CodexServerSurface.TaskUsage)
            ...
            .SelectMany(capture => capture.TaskUsage!.Threads
                ...
                .Select(task =>
                {
                    CodexLedgerEntry[] local = ledger.Where(x => x.ThreadId == task.ThreadId).ToArray();
                    bool owned = capture.CorrelatedAccountKey is not null && local.Length > 0 &&
                                 local.All(x => x.AccountKey == capture.CorrelatedAccountKey);
                    return new CodexChatAccounting(
                        task.ThreadId,
                        local.Sum(x => x.Workload.ReportedTotalTokens),
                        capture.CorrelatedAccountKey,
                        capture.CollectedAtUtc,
                        capture.TaskUsage.DataAsOfUtc,
                        task,
                        (local.Length == 0 ? "No local match in this range. " :
                            owned ? "Exact thread ID and bounded account assertions match. " :
    ...
    public static IReadOnlyList<CodexPeriodReconciliation> Analyze(
        IEnumerable<CodexServerObservation> observations,
        IReadOnlyList<CodexLedgerEntry> ledger,
        ...
        DateTimeOffset now)
    {
        ...
        CodexServerObservation[] retained = observations.ToArray();
        foreach (CodexServerObservation newest in LatestAttempts(retained, CodexServerSurface.PlanHistory))
        {
            string? accountKey = newest.CorrelatedAccountKey;
            IEnumerable<CodexServerObservation> account = retained.Where(x =>
                x.Surface == newest.Surface && x.ClientVersion == newest.ClientVersion && x.CorrelatedAccountKey == accountKey);
            if (newest.PlanHistory is null || newest.State is not (ServerEvidenceState.Available or ServerEvidenceState.Empty)) continue;
            CodexPlanHistoryReport report = newest.PlanHistory;
            foreach (CodexPlanPeriod period in
                     report.Periods.Where(x => x.StartsAtUtc < selection.ToUtc && x.EndsAtUtc > selection.FromUtc))
            {
                CodexPlanPeriod?[] history = account.Where(x => x.PlanHistory is not null).OrderBy(x => x.CollectedAtUtc)
                    .Select(x => x.PlanHistory!.Periods.FirstOrDefault(p => p.Id == period.Id)).Where(x => x is not null).ToArray();
                int revisions = history.Zip(history.Skip(1))
                    .Count(x => JsonSerializer.Serialize(x.First) != JsonSerializer.Serialize(x.Second));
                ...
                foreach (CodexPlanBreakdown group in period.Breakdowns)
                {
                    if (group.Rows.Select(x => x.Key).Distinct().Count() != group.Rows.Count)
                    {
                        checks.Add($"{group.Dimension}: duplicate keys, conservation unavailable");
                        continue;
                    }
                    if (period.UsedBasisPoints is { } used)
                    {
                        decimal difference = group.Rows.Sum(x => x.BasisPoints) - used;
                        checks.Add(
                            $"{group.Dimension}: partition minus period = {difference / 100m:0.####} pp; dimensions are alternatives, never added together");
                        if (Math.Abs(difference) > 1m) states.Add("Conflicting partition (>1 basis point diagnostic threshold)");
                    }
                }
                bool scopeComplete = !selection.HasWorkFilter && selection.FromUtc <= period.StartsAtUtc &&
                                     selection.ToUtc >= period.EndsAtUtc;
                if (!scopeComplete) checks.Add("Selection does not contain this whole unfiltered period; no whole-period comparison.");
                long tokens = ledger.Where(x => accountKey is not null && x.AccountKey == accountKey &&
                                                x.ObservedAtUtc >= period.StartsAtUtc && x.ObservedAtUtc < period.EndsAtUtc)
                    .Sum(x => x.Workload.ReportedTotalTokens);
                checks.Add("Local tokens include only matching bounded account assertions; missing ownership is excluded, not estimated.");
                int tolerance = report.BoundaryToleranceSeconds ?? 0;
                QuotaSnapshot[] candidates = quota.Where(x => scopeComplete && accountKey is not null && x.AccountKey == accountKey &&
                                                              x.Authority == QuotaObservationAuthority.ProviderAuthoritative &&
                                                              x.PlanType == period.PlanType &&
                ...
                IGrouping<QuotaHistoryCohort, QuotaSnapshot>[] lanes = candidates.GroupBy(QuotaHistoryPolicy.Cohort).ToArray();
                QuotaSnapshot? last = lanes.Length == 1 ? lanes[0].OrderByDescending(x => x.CapturedAtUtc).First() : null;
                checks.Add(
                    last is null
                        ? "No unique compatible live-meter lane with historical boundary/as-of support."
                        : $"Last compatible meter at {last.CapturedAtUtc:O}; not an integrated full-period total or proof of final agreement.");
                result.Add(
                    new CodexPeriodReconciliation(
                        period.Id,
                        period.StartsAtUtc,
                        period.EndsAtUtc,
                        period.UsedBasisPoints / 100m,
                        report.DataAsOfUtc,
                        period.AccountingComplete,
                        history.Length,
                        revisions,
                        tokens,
                        last?.UsedPercent,
