...
internal sealed class CodexRolloutComparisonReader(Action? afterRead = null)
{
        ...
        string alternatePath, string indexedPath, string threadId, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var alternateStamp = Stamp(alternatePath);
            var indexedStamp = Stamp(indexedPath);
            ...
            var alternate = await ReadAsync(alternatePath, alternateStamp.Length, cancellationToken);
            var indexed = await ReadAsync(indexedPath, indexedStamp.Length, cancellationToken);
            afterRead?.Invoke();
            ...
            var owned = left.OwnedStart is not null && right.OwnedStart is not null;
            if (alternate.AsSpan().SequenceEqual(indexed))
                return new(alternatePath, indexedPath, CodexRolloutComparisonKind.IdenticalBytes,
                    owned, hasPrefix, owned ? left.Records.Count : null,
            ...
            {
                cancellationToken.ThrowIfCancellationRequested();
                var l = left.Records[common];
                var r = right.Records[common];
                if (!alternate.AsSpan(l.Start, l.Length).SequenceEqual(indexed.AsSpan(r.Start, r.Length))) break;
                common++;
            }
            var kind = common == left.Records.Count && common == right.Records.Count
            ...
            {
                CodexRolloutComparisonKind.IdenticalOwnedRecords => "Owned record bytes match; whole files differ outside that owned stream. Whole files are not identical.",
                CodexRolloutComparisonKind.PrefixOverlap => "One complete owned record stream is an exact prefix of the other. Extra records are not automatically missing collected work.",
                _ => "Owned record streams differ after the common prefix. This is byte-level divergence, not proof of a semantic conflict or additional billable work."
            };
            return new(alternatePath, indexedPath, kind, true, hasPrefix, common, detail + " No records were imported or merged.");
        }
    ...
    private static RecordScan InspectRecords(byte[] bytes, string path, string threadId, CancellationToken cancellationToken)
    {
        ...
        int? ownedStart = null;
        var hasPrefix = false;
        var filenameOwner = CodexRolloutParser.ExtractSessionIdFromFileName(path);
        var corroboratedFilename = string.Equals(filenameOwner, threadId, StringComparison.OrdinalIgnoreCase);
        if (bytes.Length == 0 || bytes[^1] != (byte)'\n') return new(null, false, result, "empty or incomplete final record");
            ...
            while (start < bytes.Length)
            {
                ...
                if (root.TryGetProperty("type", out var type) && type.ValueKind == JsonValueKind.String && string.Equals(type.GetString(), "session_meta", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (corroboratedFilename && string.Equals(id.GetString(), threadId, StringComparison.OrdinalIgnoreCase))
                        ...
                        return new(null, hasPrefix, result, "conflicting session owner after ownership began");
                    else
                        hasPrefix = true;
                }
        ...
        catch (JsonException) { return new(null, hasPrefix, result, "malformed or unsupported JSON"); }
        return new(ownedStart, hasPrefix, result, null);
    }
}
