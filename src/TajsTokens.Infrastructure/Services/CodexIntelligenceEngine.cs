...
public sealed class CodexIntelligenceEngine(Func<ICodexForecastDatasetReader> datasetReader, ICodexServerEvidenceReader serverEvidence,
    IIntelligenceService intelligence, Func<TelemetrySnapshot> current) : ICodexIntelligence
{
    ...
    public Task<CodexIntelligenceSnapshot> AnalyzeAsync(CodexIntelligenceSnapshot snapshot, CancellationToken token) => Task.Run(async () =>
    {
        ...
        var data = await datasetReader().ReadAsync("codex", "default",
    ...
    private async Task<CodexIntelligenceSnapshot> ReadAsync(CodexSelection selection, CancellationToken token)
    {
        ...
        var dataset = await datasetReader().ReadAsync("codex", "default",
            selection.FromUtc, selection.ToUtc, token, selection.AccountKey, includeLedger: true);
        var selected = CodexIntelligenceProjection.Select(dataset.Ledger, selection);
        var history = await serverEvidence.ReadHistoryAsync(selection.FromUtc, asOf, selection.AccountKey, token);
        var evidence = history.Observations;
        var evidenceTruncated = history.IsPartial;
        var invalidEvidence = history.InvalidCaptures;
        ...
        var health = new List<CodexEvidenceHealth>
        {
            ...
            new("Snapshot", "Multiple source captures", $"Local ledger and forecast dataset captured together at {dataset.CapturedAtUtc:O}; provider history upper bound {history.CapturedAtUtc:O}. Not an atomic cross-source snapshot."),
