// Taj's Tokens | SqliteCodexObservatoryReadModel.cs
...
public sealed class SqliteCodexObservatoryReadModel(string databasePath)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        SqliteCommand agentsCommand = connection.CreateCommand();
        ...
        await using (SqliteDataReader reader = await agentsCommand.ExecuteReaderAsync(cancellationToken))
        {
            while (await reader.ReadAsync(cancellationToken))
            {
                agents.Add(
                    new Agent(
                        ...
                        reader.GetString(1),
                        reader.GetString(2),
                        Enum.TryParse<AgentRuntimeState>(reader.GetString(3), out AgentRuntimeState state)
                            ? state
        ...
        SqliteCommand relationshipsCommand = connection.CreateCommand();
        ...
        await using (SqliteDataReader reader = await relationshipsCommand.ExecuteReaderAsync(cancellationToken))
        {
            while (await reader.ReadAsync(cancellationToken))
            {
                relationships.Add(
