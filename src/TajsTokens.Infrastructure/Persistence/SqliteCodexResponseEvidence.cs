...
internal static class SqliteCodexResponseEvidence
{
    ...
    public static async Task WriteAsync(SqliteConnection connection, SqliteTransaction? transaction,
        CodexResponseObservation row, string safeFileLabel, CancellationToken cancellationToken)
    {
        ...
        string? Snapshot(CodexResponseSnapshot? value) => value is null ? null : JsonSerializer.Serialize(value, JsonOptions);
    ...
    public static async Task<CodexResponseEvidencePage> ReadAsync(SqliteConnection connection, string threadId,
        int take, CancellationToken cancellationToken)
    {
        ...
        command.CommandText = """
            ...
            """;
        command.Parameters.AddWithValue("$thread", threadId); command.Parameters.AddWithValue("$take", take + 1);
        var rows = new List<CodexResponseEvidenceRow>(); var scanned = 0; var corrupt = 0;
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ...
            try
            {
                ...
                rows.Add(new(new(reader.GetString(0), reader.GetString(1), reader.GetString(2), reader.GetInt64(3),
                    reader.GetInt64(4), reader.GetString(5), Text(6) is { } time ? DateTimeOffset.Parse(time, CultureInfo.InvariantCulture) : null,
                    DateTimeOffset.Parse(reader.GetString(7), CultureInfo.InvariantCulture), Text(8), Text(9), Text(10), Text(11), Text(12),
                    Snapshot(13), Snapshot(14), Snapshot(15), reader.GetString(16), reader.GetString(17)), reader.GetInt64(18) != 0));
            }
            catch (Exception ex) when (ex is JsonException or FormatException or InvalidCastException)
            { corrupt++; }
        }
        return new(rows, scanned > take, corrupt);
    }
}
