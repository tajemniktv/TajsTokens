// Taj's Tokens | SqliteTelemetryRepository.cs
...
public sealed partial class SqliteTelemetryRepository(string databasePath) : ITelemetryRepository, ISessionIngestionCheckpointStore
{
    ...
    public async Task<FileIngestionCheckpoint?> GetCheckpointAsync(string filePath, CancellationToken cancellationToken)
    {
        ...
        return new FileIngestionCheckpoint(
            ...
            reader.GetString(4),
            reader.IsDBNull(5) ? null : reader.GetString(5)) { ConsumedPrefixSha256 = reader.IsDBNull(6) ? null : reader.GetString(6) };
    }

    public Task SaveCheckpointAsync(FileIngestionCheckpoint checkpoint, CancellationToken cancellationToken)
    {
        return ExecutePreparedCommandAsync(
            """
    ...
    public Task UpsertForecastSnapshotAsync(ForecastSnapshot snapshot, CancellationToken cancellationToken)
    {
        return ExecutePreparedCommandAsync(
            ...
            cmd =>
            {
                Forecast forecast = snapshot.Forecast;
                ...
                cmd.Parameters.AddWithValue(
                    "$exhaustion",
                    forecast.EstimatedExhaustionAtUtc is null ? DBNull.Value : SerializeUtc(forecast.EstimatedExhaustionAtUtc.Value));
                ...
                cmd.Parameters.AddWithValue(
                    "$quotaCaptured",
                    snapshot.QuotaCapturedAtUtc is null ? DBNull.Value : SerializeUtc(snapshot.QuotaCapturedAtUtc.Value));
                cmd.Parameters.AddWithValue("$quotaWindow", DbValue(snapshot.QuotaWindowMinutes));
                cmd.Parameters.AddWithValue(
                    "$quotaReset",
                    snapshot.QuotaResetsAtUtc is null ? DBNull.Value : SerializeUtc(snapshot.QuotaResetsAtUtc.Value));
                cmd.Parameters.AddWithValue(
                    "$evaluation",
                    forecast.Evidence is null ? DBNull.Value : ForecastEvidenceJson.Serialize(forecast.Evidence));
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (await reader.ReadAsync(cancellationToken))
        {
            ForecastState state = !reader.IsDBNull(9) && Enum.TryParse<ForecastState>(reader.GetString(9), out ForecastState parsedState)
                ? parsedState
    ...
    public async Task InitializeAsync(CancellationToken cancellationToken)
    {
        ...
        SqliteCommand journalMode = connection.CreateCommand();
        journalMode.CommandText = "PRAGMA journal_mode=WAL;";
        string? activeJournalMode = Convert.ToString(await journalMode.ExecuteScalarAsync(cancellationToken), CultureInfo.InvariantCulture);
        ...
        int version = Convert.ToInt32(await versionCommand.ExecuteScalarAsync(cancellationToken), CultureInfo.InvariantCulture);
        if (version > CurrentSchemaVersion)
        {
            throw new InvalidOperationException(
                $"Telemetry database schema {version} is newer than supported version {CurrentSchemaVersion}.");
        }
        ...
        if (version == 6)
        {
            ...
            bool hasQuotaSnapshots = await TableExistsAsync(connection, "quota_snapshots", cancellationToken);
            string quotaMigration = hasQuotaSnapshots
                ? """
    ...
    public async Task InitializeIntelligenceAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException(
                    $"Intelligence schema {version} is newer than supported version {IntelligenceSchemaVersion}.");
            }
            if (version < 2)
            {
                await ExecuteMigrationAsync(
            ...
            if (version < 5)
            {
                ...
                await RefreshResetEventsAsync(connection, transaction, true, cancellationToken);
                SqliteCommand updateVersion = connection.CreateCommand();
    ...
    public async Task<int> UpsertQuotaResetEventAsync(QuotaResetEvent resetEvent, CancellationToken cancellationToken)
    {
        ...
        int inserted = await WriteQuotaResetEventAsync(connection, transaction, resetEvent, cancellationToken);
        transaction.Commit();
        return inserted;
    }

    private static async Task<int> WriteQuotaResetEventAsync(
        SqliteConnection connection,
        SqliteTransaction transaction,
        QuotaResetEvent resetEvent,
        CancellationToken cancellationToken)
    {
        ...
        bool existed = await existsCommand.ExecuteScalarAsync(cancellationToken) is not null;
        ...
        command.Parameters.AddWithValue(
            "$previousReset",
            resetEvent.PreviousResetAtUtc is null ? DBNull.Value : SerializeUtc(resetEvent.PreviousResetAtUtc.Value));
        command.Parameters.AddWithValue(
            "$currentReset",
            resetEvent.CurrentResetAtUtc is null ? DBNull.Value : SerializeUtc(resetEvent.CurrentResetAtUtc.Value));
    ...
    public async Task<int> RefreshQuotaResetEventsAsync(CancellationToken cancellationToken)
    {
        ...
        bool rebuild = Convert.ToInt32(await state.ExecuteScalarAsync(cancellationToken), CultureInfo.InvariantCulture) != 0;
        int count = await RefreshResetEventsAsync(connection, transaction, rebuild, cancellationToken);
        transaction.Commit();
        return count;
    }

    private static async Task<int> RefreshResetEventsAsync(
        SqliteConnection connection,
        SqliteTransaction transaction,
        bool rebuild,
        CancellationToken cancellationToken)
