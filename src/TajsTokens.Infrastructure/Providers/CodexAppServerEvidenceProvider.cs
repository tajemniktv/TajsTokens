// Taj's Tokens | CodexAppServerEvidenceProvider.cs
...
public sealed class CodexAppServerEvidenceProvider : ICodexServerEvidenceProvider
{
    ...
    public async Task<CodexServerCollection> CollectAsync(IReadOnlyList<string> threadIds, CancellationToken cancellationToken)
    {
        ...
        {
            StartInfo = ExternalProcess.CreateStartInfo(
                CodexAppServerQuotaProvider.ResolveCodexCommand(),
        ...
        try
        {
            ...
            async Task<string> Request(string method, object? parameters = null)
            {
                ...
                for (int lines = 0; lines < 1000; lines++)
                {
                    ...
                    if (doc.RootElement.TryGetProperty("id", out JsonElement responseId) && responseId.ValueKind == JsonValueKind.Number &&
                        responseId.TryGetInt32(out int n) && n == requestId)
                        return line;
                }
                throw new IOException("App-server notification bound exceeded.");
            }

            string initialized = await Request(
                "initialize",
                new { clientInfo = new { name = "tajs-tokens", title = "TajsTokens", version = "0.1" } });
            using (JsonDocument doc = JsonDocument.Parse(initialized))
            {
                JsonElement result = doc.RootElement.GetProperty("result");
                if (result.TryGetProperty("userAgent", out JsonElement agent))
                {
            ...
            DateTimeOffset activityStarted = DateTimeOffset.UtcNow;
            observations.Add(
                CodexServerEvidenceParser.ParseUsage(
                    await Request("account/usage/read"),
                    null,
                    activityStarted,
                    DateTimeOffset.UtcNow,
                    version));
            foreach (string thread in threadIds.Distinct(StringComparer.Ordinal))
            {
                DateTimeOffset threadStarted = DateTimeOffset.UtcNow;
                observations.Add(
                    CodexServerEvidenceParser.ParseUsage(
                        await Request("account/usage/read", new { threadId = thread }),
                        thread,
                        threadStarted,
                        DateTimeOffset.UtcNow,
                        version));
        ...
        catch (Exception e) when (e is not OutOfMemoryException)
        {
            ...
            string detail = e is OperationCanceledException
                ? "App-server evidence read timed out; no credentials retained."
            ...
            if (!observations.Any(x => x.Surface == CodexServerSurface.AccountActivity))
                observations.Add(
                    CodexServerEvidenceParser.New(CodexServerSurface.AccountActivity, null, started, DateTimeOffset.UtcNow, version)
                        with
                        {
                            State = ServerEvidenceState.Error, Detail = detail,
                        });
            foreach (string thread in threadIds.Distinct().Where(t => !observations.Any(x => x.ThreadId == t)))
            {
                observations.Add(
                    CodexServerEvidenceParser.New(CodexServerSurface.ThreadUsage, thread, started, DateTimeOffset.UtcNow, version)
                        with
                        {
                            State = ServerEvidenceState.Error, Detail = detail,
                        });
        ...
        {
            AccountBracket = new CodexServerAccountBracket(before, beforeCollected, after, afterCollected),
        }).ToList();
        foreach (CodexServerSurface surface in new[] { CodexServerSurface.PlanHistory, CodexServerSurface.GroupedAnalytics })
        {
            observations.Add(
                CodexServerEvidenceParser.New(surface, null, started, DateTimeOffset.UtcNow, version) with
                {
                    State = ServerEvidenceState.NoSupportedSeam,
                    Detail =
                    "No method in installed/upstream app-server contract. TUI backend authentication remains owned by Codex; no direct request attempted.",
                });
        }
        return new CodexServerCollection(observations);
    }
    ...
    public static CodexServerObservation Correlate(CodexServerObservation observation, string? before, string? after)
    {
        return before is not null && after is not null && before != after
            ? observation with
            {
                AccountEvidence = AccountEvidenceClass.Conflicting,
    ...
    private static string? ReadAccount(string response)
    {
        try
        {
            return CodexAppServerQuotaProvider.ParseQuotaResponse(response, DateTimeOffset.UtcNow).AccountKey;
        }
