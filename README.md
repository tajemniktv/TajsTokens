...
Source-derived implementation references may live under `docs/reference/` when they save repeated archaeology without becoming architecture authority. The current [`Codex upstream SQLite schema reference`](docs/reference/CODEX_UPSTREAM_SQLITE_SCHEMA.md) maps a pinned public Codex source snapshot and is explicitly subordinate to `PROJECT.md` and the installed runtime evidence.
