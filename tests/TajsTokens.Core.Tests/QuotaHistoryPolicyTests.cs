// Taj's Tokens | QuotaHistoryPolicyTests.cs
...
public sealed class QuotaHistoryPolicyTests
{
    ...
    {
        QuotaSnapshot rollout = Point(0, 12.4);
        QuotaSnapshot app = rollout with
        {
        ...
        IReadOnlyList<QuotaHistoryDecision> decisions = QuotaHistoryPolicy.Describe([rollout, app, Point(1, 14.2)], Start.AddHours(2));
    ...
    {
        IReadOnlyList<QuotaHistoryDecision> decisions = QuotaHistoryPolicy.Describe(
            [
                Point(0, 10), Point(0.5, 10), Point(1, 12),
                Point(1.5, 13), Point(1.5, 15) with { ObservationId = "alternative" }, Point(2, 16), Point(2.5, 18),
            ],
            Start.AddHours(3));
        Assert.Single(decisions, x => x.Reason == "possibly-cached-repeat");
        Assert.Equal(2, decisions.Count(x => x.Reason == "same-time-conflict"));
        IReadOnlyList<IReadOnlyList<QuotaSnapshot>> epochs =
            QuotaForecastCalibration.SplitEpochs(QuotaHistoryPolicy.ReplayRows(Assert.Single(QuotaHistoryPolicy.Streams(decisions))));
    ...
    {
        QuotaSnapshot row = Point(0, 10);
        QuotaSnapshot[] rows = new[]
        {
            row,
            row with { AccountKey = "A" },
            row with { LimitId = "other" },
            row with { PlanType = "other" },
            row with { WindowMinutes = 600 },
            row with { SessionId = "other" },
        };
    ...
    {
        QuotaSnapshot row = Point(0, 10);
        QuotaSnapshot[] variants = new[]
        {
            row with { ObservationId = null },
            row with { HasSourceTimestamp = false },
            row with { ResetsAtUtc = Start.AddSeconds(-1) },
            row with { UsedPercent = double.NaN },
            row with { CapturedAtUtc = Start.AddDays(1) },
            row with { Kind = QuotaWindowKind.Unknown },
        };
        QuotaHistoryDecision[] reasons = variants.Select(x => Assert.Single(QuotaHistoryPolicy.Describe([x], Start))).ToArray();
        ...
        Assert.Equal(
            ...
            reasons.Select(x => x.Reason));
    }

    [Fact]
    public void CollectionReplayCannotUseBackfilledQuotaAtHistoricalOrigins()
    {
        QuotaSnapshot[] rows = new[] { Point(0, 10), Point(0.5, 12), Point(1, 14), Point(1.5, 16) }
        ...
        QuotaSnapshot[] strict = QuotaHistoryPolicy.AvailableRows(rows, ForecastReplayAvailability.CollectedByOrigin);
    ...
    public void ParserRetainsNativeQuotaIdentityPrecisionPlanAndUnknownWindow()
    {
        ...
        parser.Parse(
            new RawSessionRecord(
                file,
                0,
                100,
                $$$"""{"timestamp":"2026-09-01T10:00:00Z","type":"session_meta","payload":{"id":"{{{session}}}"}}"""),
            state);
        var record = new RawSessionRecord(
            file,
            100,
            500,
        ...
        IReadOnlyList<QuotaSnapshot> parsed = parser.Parse(record, state).QuotaSnapshots;
        Assert.Equal(2, parsed.Count);
        Assert.All(
            parsed,
            x =>
            {
                Assert.Equal("generation", x.SourceIdentity);
                Assert.Equal(session, x.SessionId);
                Assert.Equal("codex-extra", x.LimitId);
                Assert.Equal("pro", x.PlanType);
                Assert.Null(x.AccountKey);
                Assert.NotNull(x.CollectedAtUtc);
                Assert.NotNull(x.ObservationId);
            });
    ...
    private static QuotaSnapshot Point(double hours, double used)
    {
        ...
        {
            ObservationId = "record:" + hours,
            SourceIdentity = "generation",
            SessionId = "session",
            LimitId = "codex",
            PlanType = "pro",
            Lane = "primary",
            CollectedAtUtc = Start.AddHours(hours),
            HasSourceTimestamp = true,
        };
    }

    private static CodexForecastDataset Data(QuotaSnapshot[] rows)
    {
        return new CodexForecastDataset(rows, [], [], [], Start.AddHours(2), "fixture");
    }
}