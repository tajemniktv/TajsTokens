// Taj's Tokens | CodexScenarioHistoryTests.cs
...
public sealed class CodexScenarioHistoryTests
{
    ...
    {
        DateTimeOffset start = DateTimeOffset.Parse("2026-09-01T12:00:00Z");
        ...
        QuotaSnapshot Quota(int minutes, double used, int reset = 300, string source = "codex-app-server:test")
        {
            return new QuotaSnapshot(
                ...
                used,
                300,
                start.AddMinutes(reset),
                "codex",
                "default",
                source);
        }

        var root = new CodexWorkloadObservation(
            "meta",
            "file",
            "safe.jsonl",
            0,
        ...
        var data = new CodexForecastDataset(
            ...
            [root],
            [token, token with { ObservedAtUtc = start.AddMinutes(10), Model = "future" }],
            [],
            start.AddHours(2),
            "fixture");
        IReadOnlyList<ScenarioHistorySample> samples = CodexScenarioHistoryBuilder.Build(data);
