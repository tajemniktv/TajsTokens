// Taj's Tokens | QuotaForecastBacktesterTests.cs
...
public sealed class QuotaForecastCalibrationTests
{
    ...
    {
        QuotaSnapshot[] steady = Enumerable.Range(0, 49).Select(i => Point(i / 12d, 10 + i)).ToArray();
        QuotaSnapshot[] jitter = steady.Select((row, i) => row with { ResetsAtUtc = row.ResetsAtUtc!.Value.AddSeconds(i % 2) }).ToArray();
        Assert.Single(QuotaForecastCalibration.SplitEpochs(jitter));
        Forecast forecast = QuotaPredictionService.BuildResetOutlook(jitter, jitter[^1].CapturedAtUtc);
        ...
        foreach (string model in new[] { "persistence", "epoch", "legacy-ewma" })
        {
            IReadOnlyList<QuotaForecastTrial> expected = QuotaForecastCalibration.Replay(steady, model, 0.5);
            IReadOnlyList<QuotaForecastTrial> actual = QuotaForecastCalibration.Replay(jitter, model, 0.5);
            Assert.NotEmpty(actual);
            Assert.Equal(
                expected.Select(x => (x.OriginUtc, x.OutcomeUtc, x.PredictedRemaining, x.ObservedRemaining)),
            ...
            IReadOnlyList<QuotaForecastTrial> prefix = QuotaForecastCalibration.Replay(jitter.Take(25).ToArray(), model, 0.5);
    ...
    {
        QuotaSnapshot[] drift = Enumerable.Range(0, 4).Select(i => Point(i, 10 + i) with { ResetsAtUtc = Start.AddHours(5).AddSeconds(i) })
            .ToArray();
        Assert.Equal(new[] { 2, 2 }, QuotaForecastCalibration.SplitEpochs(drift).Select(x => x.Count));
        QuotaSnapshot first = Point(0, 50);
        foreach (QuotaSnapshot changed in new[]
                 {
                     ...
                     Point(5, 60) with
                     {
                         CapturedAtUtc = Start.AddHours(5).AddMilliseconds(500), ResetsAtUtc = Start.AddHours(5).AddSeconds(1),
                     },
    ...
    {
        QuotaForecastTrial[] trials = Enumerable.Range(0, 10).Select(i => new QuotaForecastTrial(
            "epoch",
            "2h",
            Start.AddDays(-2).AddMinutes(i),
            Start.AddDays(-2).AddHours(2).AddMinutes(i),
            Start.AddDays(-1).AddSeconds(i % 2),
            2,
            60,
            50,
            null,
        ...
        IReadOnlyList<double> errors = QuotaForecastCalibration.CalibrationErrors(trials, Start, Start.AddHours(5), 2);
    ...
    {
        QuotaSnapshot[] rows = new[] { Point(0, 10), Point(1, 50), Point(2, 5), Point(3, 15) };
        IReadOnlyList<IReadOnlyList<QuotaSnapshot>> epochs = QuotaForecastCalibration.SplitEpochs(rows);
        Assert.Equal(2, epochs.Count);
        Assert.Equal(10, QuotaPaceModels.Estimate(epochs[1], "epoch"));
        Forecast forecast = QuotaPredictionService.BuildResetOutlook(rows, Start.AddHours(3));
    ...
    {
        QuotaSnapshot[] sparse = new[] { Point(0, 10), Point(1, 20), Point(2, 30) };
        QuotaSnapshot[] dense = sparse.Append(Point(0.9, 10)).Append(Point(1.9, 20)).OrderBy(x => x.CapturedAtUtc).ToArray();
    ...
    {
        QuotaForecastTrial Trial(DateTimeOffset reset, DateTimeOffset outcome, double error)
        {
            return new QuotaForecastTrial(
                "epoch",
                "2h",
                outcome.AddHours(-2),
                outcome,
        ...
        IReadOnlyList<double> resetErrors = QuotaForecastCalibration.CalibrationErrors(
            trials.Select(x => x with { Target = "near-reset-proxy" }),
            Start,
            Start.AddHours(5),
            2);
    ...
    {
        QuotaSnapshot[] rows = Enumerable.Range(0, 9).Select(i => Point(i * 0.5, 10 + i)).ToArray();
        IReadOnlyList<QuotaForecastTrial> trials = QuotaForecastCalibration.Replay(rows, "epoch", 0.5);
    ...
    {
        Forecast forecast = QuotaPredictionService.BuildResetOutlook(
            [Point(0, 10) with { Source = "embedded" }, Point(1, 50)],
            Start.AddHours(1));
