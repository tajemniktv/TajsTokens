// Taj's Tokens | CodexThreadNavigationTests.cs
...
public sealed class CodexThreadNavigationTests
{
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-navigation-");
        try
        {
            string path = Path.Combine(directory.FullName, "state_1.sqlite");
            await CreateNavigationDbAsync(
                path,
                """
            ...
            CodexThreadNavigationResult result = await new CodexThreadObservabilityService(directory.FullName)
                .BrowseThreadsAsync(new CodexThreadNavigationQuery(), CancellationToken.None);

            CodexThreadNavigationGroup group = Assert.Single(result.Groups);
            ...
            CodexThreadNavigationNode root = Assert.Single(group.RootThreads);
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-navigation-search-");
        try
        {
            string path = Path.Combine(directory.FullName, "state_1.sqlite");
            await CreateNavigationDbAsync(
            ...
            CodexThreadNavigationGroup group = Assert.Single(result.Groups);
            CodexThreadNavigationNode node = Assert.Single(group.RootThreads);
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-navigation-cycle-");
        try
        {
            string path = Path.Combine(directory.FullName, "state_1.sqlite");
            await CreateNavigationDbAsync(
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-navigation-archived-");
        try
        {
            string path = Path.Combine(directory.FullName, "state_1.sqlite");
            await CreateNavigationDbAsync(
            ...
            CodexThreadNavigationResult current = await service.BrowseThreadsAsync(
                new CodexThreadNavigationQuery(),
                CancellationToken.None);
            CodexThreadNavigationResult all = await service.BrowseThreadsAsync(
                new CodexThreadNavigationQuery(IncludeArchived: true),
                CancellationToken.None);
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-navigation-groups-");
        try
        {
            string path = Path.Combine(directory.FullName, "state_1.sqlite");
            await CreateNavigationDbAsync(
            ...
            var service = new CodexThreadObservabilityService(directory.FullName);
            CodexThreadNavigationResult all = await service.BrowseThreadsAsync(new CodexThreadNavigationQuery(), CancellationToken.None);
            CodexThreadNavigationResult searched = await service.BrowseThreadsAsync(
                new CodexThreadNavigationQuery("reviewer"),
                CancellationToken.None);
    ...
    {
        return nodes.SelectMany(node => new[] { node }.Concat(Flatten(node.Children)));
    }
