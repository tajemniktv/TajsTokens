// Taj's Tokens | QuotaCostEvaluationTests.cs
...
public sealed class QuotaCostEvaluationTests
{
    ...
    public void LaterWorkCannotIdentifyAnEmptyFrozenTrainingPrefix()
    {
        ...
        DateTimeOffset cutoff = original.Quota[0].CapturedAtUtc.AddMinutes(615); // End of the first twenty half-hour targets.
        CodexForecastDataset data = original with { Tokens = original.Tokens.Where(x => x.ObservedAtUtc > cutoff).ToArray() };
        Assert.NotEmpty(data.Tokens);
        QuotaCostScore[] cost = QuotaCostEvaluation.Evaluate(data).Scores.Where(x => x.HorizonHours == .5).ToArray();
        Assert.All(cost.Where(x => x.Candidate is "pace" or "persistence"), x => Assert.NotEmpty(x.Trials));
        Assert.All(
            cost.Where(x => x.Candidate is not ("pace" or "persistence")),
            x =>
            {
        ...
        ComposedQuotaEvaluation composed = ComposedQuotaEvaluator.Evaluate(data, horizons: [.5]);
        Assert.All(
            composed.Scores,
            x =>
            {
        ...
        SessionQuotaScore session = SessionQuotaEvaluator.Evaluate(data).Scores.Single(x => x.HorizonHours == .5);
    ...
    {
        CodexForecastDataset original = ComposedQuotaEvaluatorTests.TimelyData();
        CodexForecastDataset data = original with { Quota = original.Quota.Select(x => x with { UsedPercent = 20 }).ToArray() };
        QuotaCostScore total = QuotaCostEvaluation.Evaluate(data).Scores.Single(x => x.HorizonHours == .5 && x.Candidate == "total");
    ...
    {
        CodexForecastDataset data = Data(Enumerable.Range(0, 13).Select(i => Quota(i * 15, 20)).ToArray(), []);
        QuotaCostObservationBuild result = QuotaCostObservationBuilder.BuildDetailed(data, horizons: [.5]);
        QuotaCostConstructionCoverage coverage = Assert.Single(result.Coverage);
    ...
    {
        CodexForecastDataset data = Data(new[] { 0, 5, 15, 45, 60, 90 }.Select(i => Quota(i, 20)).ToArray(), []);
        QuotaCostObservationBuild result = QuotaCostObservationBuilder.BuildDetailed(data, horizons: [.5]);
        QuotaCostConstructionCoverage coverage = Assert.Single(result.Coverage);
        ...
        QuotaCostObservationBuild empty = QuotaCostObservationBuilder.BuildDetailed(Data([], []));
    ...
    {
        QuotaSnapshot[] quota = Enumerable.Range(0, 13).Select(i => Quota(i * 15, 20)).ToArray();
        CodexPredictiveTokenEvent token = Token(15, 100);
        CodexForecastDataset data = Data(quota, [token, Token(16, 200), Token(45, 300), Token(46, 999)]);
        IReadOnlyList<QuotaCostObservation> rows = QuotaCostObservationBuilder.Build(data);
        QuotaCostObservation first = rows.First(x => x.HorizonHours == 0.5);
        ...
        foreach (IGrouping<double, QuotaCostObservation> horizon in rows.GroupBy(x => x.HorizonHours))
    ...
    {
        QuotaSnapshot[] quota = Enumerable.Range(0, 13).Select(i => Quota(i * 15, 20 + i)).ToArray();
        quota[4] = quota[4] with { UsedPercent = null };
        quota[8] = quota[8] with { PlanType = "other" };
        IReadOnlyList<QuotaCostObservation> rows = QuotaCostObservationBuilder.Build(Data(quota, []));
    ...
    {
        QuotaSnapshot[] quota = Enumerable.Range(0, 9).Select(i => Quota(i * 15, 20) with
        {
            Source = "codex-rollout:primary",
            SourceIdentity = "file",
            SessionId = "session",
            ObservationId = $"row-{i}",
            AccountKey = null,
        ...
        QuotaCostObservation row = QuotaCostObservationBuilder.Build(Data(quota, [])).First();
        ...
        QuotaCostObservation confirmed = QuotaCostObservationBuilder.Build(Data(quota, []), userConfirmedRolloutOwnership: true).First();
    ...
    {
        QuotaSnapshot[] quota = Enumerable.Range(0, 9).Select(i => Quota(i * 15, 90 + i)).ToArray();
        quota[3] = quota[3] with { UsedPercent = 100 };
        quota[4] = quota[4] with { UsedPercent = 1 };
        IReadOnlyList<QuotaCostObservation> rows = QuotaCostObservationBuilder.Build(Data(quota, []));
        Assert.DoesNotContain(rows, x => x.EndUsed >= 100);
        QuotaCostObservationBuild construction = QuotaCostObservationBuilder.BuildDetailed(Data(quota, []));
        ...
        Assert.DoesNotContain(rows, x => x.StartUtc < quota[4].CapturedAtUtc && x.EndUtc >= quota[4].CapturedAtUtc);
        quota = Enumerable.Range(0, 9).Select(i => Quota(i * 15, 20 + i) with { ResetsAtUtc = Start.AddDays(7).AddSeconds(i) }).ToArray();
        Assert.Empty(QuotaCostObservationBuilder.Build(Data(quota, [])));
    }
    ...
    {
        QuotaCostObservation[] rows = Observations(130);
        QuotaCostReport report = QuotaCostEvaluation.Evaluate(rows, "fixture");
        QuotaCostScore score = report.Scores.Single(x => x.Candidate == "total");
        Assert.True(score.BandSamples > 0);
        Assert.All(score.Trials.Take(8), x => Assert.Null(x.LowerPrediction));
        QuotaCostObservation[] oneReset = rows.Select(x => x with { ResetUtc = Start.AddDays(100) }).ToArray();
        Assert.All(
            QuotaCostEvaluation.Evaluate(oneReset, "fixture").Scores,
            x =>
            {
    ...
    {
        QuotaCostObservation[] rows = Observations(60);
        QuotaCostReport original = QuotaCostEvaluation.Evaluate(rows, "fixture");
        QuotaCostObservation[] changed = rows.Select((x, i) => i < 40
            ? x
            : x with
            {
                EndUsed = 99,
        ...
        QuotaCostReport after = QuotaCostEvaluation.Evaluate(changed, "fixture");
        foreach (QuotaCostScore score in original.Scores)
        {
            QuotaCostScore other = after.Scores.Single(x => x.Candidate == score.Candidate);
    ...
    public void EmptyLocalWorkDoesNotBecomeTimeCostAndSparseCohortsDoNotBorrow()
    {
        ...
        IEnumerable<QuotaCostObservation> unknown = rows.Take(10).Select(x => x with { Cohort = x.Cohort with { AccountKey = null } });
        QuotaCostReport report = QuotaCostEvaluation.Evaluate(rows.Concat(unknown).ToArray(), "fixture");
        Assert.All(report.Scores.Where(x => x.Cohort.AccountKey is null), x => Assert.Equal(0, x.HeldOutSamples));
        Assert.All(
            report.Scores.Where(x => x.Cohort.AccountKey is not null && x.Candidate is not "pace" and not "persistence"),
    ...
    {
        QuotaCostObservation[] rows = Observations(80);
        ...
        QuotaCostScore score = QuotaCostEvaluation.Evaluate(rows, "fixture").Scores.Single(x => x.Candidate == "total");
    ...
    {
        QuotaHistoryCohort cohort = QuotaHistoryPolicy.Cohort(Quota(0, 10));
        return Enumerable.Range(0, count).Select(i =>
        {
            DateTimeOffset time = Start.AddDays(i / 10).AddMinutes(i % 10 * 30);
            long tokens = 1_000_000L * (1 + i % 3);
            CodexForecastFeatures features = CodexForecastFeatureBuilder.Build(Data([], [Token(1, tokens)]), Start.AddMinutes(30), 0.5);
            double delta = tokens / 1e6 * 2;
    ...
    private static QuotaSnapshot Quota(int minutes, double used)
    {
        return new QuotaSnapshot(
            ...
            "account") { HasSourceTimestamp = true, CollectedAtUtc = Start.AddMinutes(minutes), PlanType = "pro" };
    }

    private static CodexPredictiveTokenEvent Token(int minutes, long value)
    {
    ...
    {
        return new CodexForecastDataset(quota, [], tokens, [], Start.AddDays(30), "fixture");
    }
}