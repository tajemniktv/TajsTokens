// Taj's Tokens | ComposedQuotaBreakdownsTests.cs
...
public sealed class ComposedQuotaBreakdownsTests
{
    ...
    {
        DateTimeOffset at = DateTimeOffset.Parse("2026-09-01T00:00:00Z");

        ComposedQuotaTrial Trial(double prediction, double? lower, double? upper)
        {
            return new ComposedQuotaTrial(
                at,
                at.AddMinutes(30),
                at.AddDays(7),
                prediction,
                ...
                new PredictedWorkload(
                    ...
                    "fixture")) { LowerObservedDelta = lower, UpperObservedDelta = upper };
        }

        ComposedQuotaTrial[] rows = new[]
        {
            ...
            Trial(0, null, null),
            Trial(0, 7, 5),
            Trial(0, double.NaN, 7),
            Trial(double.NaN, 5, 7), // Known group, unusable prediction: excluded from miss denominator.
        };
        IReadOnlyList<ComposedQuotaBreakdown> report = ComposedQuotaBreakdowns.Build(rows);
        ComposedQuotaBreakdown[] groups = report.Where(x => x.Dimension == "outcome-quota-movement").ToArray();
        Assert.Equal(rows.Length, groups.Sum(x => x.Outcomes));
        ComposedQuotaBreakdown high = Assert.Single(groups, x => x.Group == "at-least-5pp");
        ...
        ComposedQuotaBreakdown unknown = Assert.Single(groups, x => x.Group == "unknown-meter-envelope");
    ...
    public void PartitionsConserveOutcomesAndComparisonsUseMatchedSubsets()
    {
        ...
        ComposedQuotaTrial Trial(int i, double shift, Dictionary<string, double> mix)
        {
            return new ComposedQuotaTrial(
                at.AddHours(i),
                at.AddHours(i + .5),
                at.AddDays(7).AddSeconds(shift),
                1 + i * 2,
                1,
                2,
                10 + i * 10,
                ...
                new PredictedWorkload(
                    ...
                    at,
                    ForecastReplayAvailability.CollectedByOrigin,
                    "fixture")) { OriginActivity = "RecentActivity", RecordedOutcomeTokens = i == 0 ? 0 : 10 };
        }

        ComposedQuotaTrial[] trials = new[]
        {
            Trial(0, 0, new Dictionary<string, double> { ["m"] = .7 }) with
            {
                ZeroUseIntervalLoss = .5, ObservedRemainingPercent = 15, LowerRemainingPercent = 10, UpperRemainingPercent = 20,
            },
            Trial(1, .8, new Dictionary<string, double> { ["m"] = .8 }) with
            {
                CompleteOutcomeTokenCategories = false,
                OutcomeQualityFlags = ["missing-runtime", "incomplete-token-categories", "missing-runtime"],
            },
            Trial(2, 1.6, new Dictionary<string, double> { ["m"] = .5, ["n"] = .5 }) with
            {
                IncumbentIntervalLoss = 2, ObservedRemainingPercent = 25, LowerRemainingPercent = 0, UpperRemainingPercent = 10,
            },
        };
        IReadOnlyList<ComposedQuotaBreakdown> report = ComposedQuotaBreakdowns.Build(trials);
        ...
        ComposedQuotaBreakdown activity = Assert.Single(report, x => x.Dimension == "origin-activity");
        ...
        Assert.Contains(
            report,
            x => x.Dimension == "outcome-category-coverage" && x.Group == "incomplete-reported-vectors" && x.Outcomes == 1);
        Assert.Contains(report, x => x.Dimension == "outcome-category-coverage" && x.Group == "unknown" && x.Outcomes == 2);
        Assert.Contains(
            report,
            x => x.Dimension == "outcome-quality" && x.Group == "incomplete-token-categories; missing-runtime" && x.Outcomes == 1);
        Assert.Contains(
            report,
            x => x.Dimension == "outcome-quality" && x.Group == "no-recorded-flags-not-proven-complete" && x.Outcomes == 2);
    ...
    public void StrictTrialBreakdownsDoNotBorrowLateActivity()
    {
        ...
        data = data with
        {
            Tokens = data.Tokens.Select(x => x with
            {
                ObservedAtUtc = x.ObservedAtUtc.AddMinutes(-12), CapturedAtUtc = x.CapturedAtUtc?.AddMinutes(-12),
            }).ToArray(),
        };
        ...
        DateTimeOffset origin = before.Scores.SelectMany(x => x.Trials).Min(x => x.OriginUtc);
