// Taj's Tokens | ChronologicalEvidenceTests.cs
...
public sealed class ChronologicalEvidenceTests
{
    ...
    {
        Outcome[] rows = Enumerable.Range(0, 12).Select(i => new Outcome(
            Start.AddHours(i * 6),
            Start.AddHours(i * 6 + 1),
            Start.AddDays(7),
            i % 2 == 0 ? -1 : 1)).ToArray();
        ChronologicalEvidence.Support support = ChronologicalEvidence.Describe(
            rows.Concat(rows).ToArray(),
            x => x.Start,
            x => x.End,
            x => x.Reset,
            x => x.Error);
    ...
    public void ResetCrossingsAndOverlappingOutcomesAreExcluded()
    {
        Outcome[] rows =
        [
            new(Start, Start.AddHours(2), Start.AddHours(6), 1),
        ...
        IReadOnlyList<IReadOnlyList<Outcome>> blocks = ChronologicalEvidence.Blocks(rows, x => x.Start, x => x.End, x => x.Reset);
    ...
    {
        DateTimeOffset reset = Start.AddDays(7);
        QuotaForecastTrial[] rows = Enumerable.Range(0, 10).Select(i => new QuotaForecastTrial(
            "pace",
            "0.5h",
            Start.AddHours(i * 6),
            Start.AddHours(i * 6 + .5),
            reset,
            .5,
            50,
            49,
            ...
            false,
            null)).ToArray();
        DateTimeOffset origin = Start.AddDays(3);
        Assert.NotNull(QuotaForecastCalibration.ErrorRadius(QuotaForecastCalibration.CalibrationErrors(rows, origin, reset, .5)));
        Assert.Empty(
            QuotaForecastCalibration.CalibrationErrors(rows.Select(x => x with { Target = "near-reset-proxy" }), origin, reset, .5));
