// Taj's Tokens | QuotaAccountScopeTests.cs
...
public sealed class QuotaAccountScopeTests : IDisposable
{
    ...
    {
        QuotaSnapshot a = Point(Now.AddHours(-1), 90, null);
        QuotaSnapshot b = a with { CapturedAtUtc = Now, UsedPercent = 5 };
        IReadOnlyList<QuotaResetEvent> events = new QuotaResetDetector().Detect(
        [
            a, b,
        ...
        foreach (QuotaResetEvent item in events) await repository.UpsertQuotaResetEventAsync(item, default);
        await ExecuteAsync(
            "UPDATE quota_reset_events SET event_id='legacy-' || source; UPDATE intelligence_schema SET version=2 WHERE component='phase4-intelligence';");
    ...
    public async Task ResetIdentityMigrationRebuildsAllHistoryAtomicallyAndRetries()
    {
        ...
        QuotaSnapshot first = Point(Now.AddDays(-10), 90, null) with { ResetsAtUtc = Now.AddDays(-9) };
        QuotaSnapshot second = first with { CapturedAtUtc = first.CapturedAtUtc.AddMinutes(1), UsedPercent = 5 };
        foreach (string plan in new[] { "a", "b" })
        foreach (QuotaSnapshot row in new[] { first, second })
            await repository.UpsertQuotaSnapshotAsync(row with { PlanType = plan }, default);
        for (int i = 0; i < 520; i++)
            await repository.UpsertQuotaSnapshotAsync(
                Point(Now.AddMinutes(-520 + i), 20, "recent") with { ResetsAtUtc = Now.AddHours(5).AddSeconds(i % 2) },
                default);
        QuotaResetEvent legacy = Assert.Single(new QuotaResetDetector().Detect([first, second])) with { EventId = "legacy-event" };
        await repository.UpsertQuotaResetEventAsync(legacy, default);
        await ExecuteAsync(
            "UPDATE intelligence_schema SET version=3; CREATE TRIGGER fail_reset_rebuild BEFORE INSERT ON quota_reset_events BEGIN SELECT RAISE(ABORT, 'fixture failure'); END;");
    ...
    public async Task ResetCacheInvalidationRollsBackWithEvidenceAndRebuildFailureRemainsRetryable()
    {
        var repository = new SqliteTelemetryRepository(Database);
        await repository.InitializeIntelligenceAsync(default);
        QuotaSnapshot first = Point(Now.AddDays(-10), 90, "keep") with { ResetsAtUtc = Now.AddDays(-9) };
        await repository.UpsertQuotaSnapshotAsync(first, default);
        await repository.UpsertQuotaSnapshotAsync(
            first with { CapturedAtUtc = first.CapturedAtUtc.AddMinutes(1), UsedPercent = 5 },
            default);
        ...
        await ExecuteAsync(
            "DELETE FROM quota_snapshots WHERE account_key='delete'; CREATE TRIGGER fail_refresh BEFORE INSERT ON quota_reset_events BEGIN SELECT RAISE(ABORT, 'fixture failure'); END;");
    ...
    public async Task ObservatoryFirstInitializationMigratesOldQuotaSchema()
    {
        ...
        await ExecuteAsync(
            "DROP TABLE quota_snapshots; DROP TABLE forecast_snapshots; DROP TABLE ingestion_checkpoints; DROP TABLE codex_server_evidence;");
    ...
    {
        CodexQuotaResponse first = Response("\"accountId\":\"sanitized-account-A\",", true);
        CodexQuotaResponse empty = Response("\"accountId\":\"sanitized-account-A\",", false);
        CodexQuotaResponse other = Response("\"accountId\":\"sanitized-account-B\",", true);
    ...
    {
        CodexQuotaResponse response = Response(property, true);
    ...
    public async Task SameTimeAccountsAndUnknownCoexistAndFilterBeforeLimit()
    {
        var repository = new SqliteTelemetryRepository(Database);
        await repository.InitializeAsync(default);
        foreach (string? account in new[] { null, "A", "B" })
    ...
    public async Task CurrentForecastDoesNotBorrowOtherOrUnknownAccountAndForecastKeysCoexist()
    {
        ...
        QuotaSnapshot current = Point(Now, 20, "A");
        CurrentQuotaForecast result = Assert.Single(await service.BuildAndPersistCurrentForecastsAsync([Lane(current)], Now, default));
        Assert.Equal(10, result.Forecast!.BurnRatePercentPerHour);
        CurrentQuotaForecast b = Assert.Single(
            await service.BuildAndPersistCurrentForecastsAsync([Lane(current with { AccountKey = "B" })], Now, default));
        Assert.NotNull(b.Forecast);
        IReadOnlyList<ForecastSnapshot> stored = await repository.GetRecentForecastSnapshotsAsync(
            QuotaWindowKind.FiveHour,
            "codex",
            "default",
            10,
        ...
        CurrentQuotaForecast unknown = Assert.Single(
            await service.BuildAndPersistCurrentForecastsAsync([Lane(current with { AccountKey = null })], Now, default));
        Assert.Null(unknown.Forecast);
        Assert.Contains("scope", unknown.HistoryPolicy);
        Assert.Equal(
            2,
            (await repository.GetRecentForecastSnapshotsAsync(QuotaWindowKind.FiveHour, "codex", "default", 10, default)).Count);
    }

    [Fact]
    public void ResetsAndReplayNeverBridgeAccounts()
    {
        QuotaSnapshot first = Point(Now.AddHours(-1), 90, "A");
        QuotaSnapshot second = Point(Now, 5, "B");
        ...
        QuotaSnapshot[] own = new[] { Point(Now.AddHours(-1), 10, "B"), Point(Now, 20, "B") };

        Assert.Equal(QuotaPredictionService.BuildResetOutlook(own, Now), QuotaPredictionService.BuildResetOutlook([first, .. own], Now));
        IReadOnlyList<QuotaResetEvent> resets =
            new QuotaResetDetector().Detect([first, second, first with { CapturedAtUtc = Now, UsedPercent = 5 }]);
    ...
    public async Task VersionEightMigrationPreservesUnknownRowsAndIsIdempotent()
    {
        ...
        await new SqliteTelemetryRepository(Database).InitializeAsync(default);
        Assert.Null(
            Assert.Single(await repository.GetRecentQuotaSnapshotsAsync(QuotaWindowKind.FiveHour, "codex", "default", 10, default))
                .AccountKey);
        ForecastSnapshot forecast = Assert.Single(
            await repository.GetRecentForecastSnapshotsAsync(QuotaWindowKind.FiveHour, "codex", "default", 10, default));
        Assert.Null(forecast.AccountKey);
        Assert.Equal(2, forecast.Forecast.BurnRatePercentPerHour);
        Assert.Empty(
            await repository.GetRecentQuotaSnapshotsAsync(QuotaWindowKind.FiveHour, "codex", "default", 10, default, accountKey: "A"));
    ...
    {
        ScenarioHistorySample[] samples = Enumerable.Range(1, 20).Select(i => new ScenarioHistorySample(
            QuotaWindowKind.FiveHour,
            Now.AddHours(-i - 1),
            Now.AddHours(-i),
            5,
            ...
            AccountKey: "A")).ToArray();

        Assert.True(QuotaPredictionService.Simulate(new ScenarioRequest(1, 1, 0, AccountKey: "A"), samples, Now).FiveHour.HasEnoughHistory);
        Assert.False(
            QuotaPredictionService.Simulate(new ScenarioRequest(1, 1, 0, AccountKey: "B"), samples, Now).FiveHour.HasEnoughHistory);
    }

    [Fact]
    public void AccountSwitchRearmsLowAlertWithoutInventingReset()
    {
        TelemetrySnapshot Snapshot(double used, string account)
        {
    ...
    public async Task TokenForecastUsesRolloutsWithoutQuotaAccountOrQuotaRows()
    {
        ...
        using (var connection = new SqliteConnection($"Data Source={Database}"))
        {
            ...
            command.Parameters.AddWithValue("$captured", Now.ToString("O", CultureInfo.InvariantCulture));
            for (int i = 0; i < 80; i++)
            {
                command.Parameters["$id"].Value = $"fixture-{i}";
                command.Parameters["$at"].Value = Now.AddMinutes(-1190 + i * 15).ToString("O", CultureInfo.InvariantCulture);
                await command.ExecuteNonQueryAsync();
            }
            transaction.Commit();
        }
        TokenWorkloadForecast forecast = await intelligence.ForecastTokenWorkloadAsync(Now, default);
    ...
    {
        return CodexAppServerQuotaProvider.ParseQuotaResponse(
            "{\"result\":{" + accountProperty + "\"rateLimits\":{" +
            (window ? "\"primary\":{\"usedPercent\":20,\"windowDurationMins\":300}" : "") + "}}}",
            Now);
    }
    ...
    private static QuotaSnapshot Point(DateTimeOffset captured, double used, string? account)
    {
        return new QuotaSnapshot(
            ...
            account) { HasSourceTimestamp = true };
    }
