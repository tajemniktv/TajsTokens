...
public sealed class CodexIntelligenceTests
{
    ...
    public async Task LedgerUsesNativeThreadIdentityAndImmutableCapturesWithoutInventingAccountOwnership()
    {
        ...
        var directory = Path.Combine(root!.FullName, ".codex", "temp", "intelligence-test-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(directory);
        try
        {
            var database = Path.Combine(directory, "telemetry.db");
            var repository = new SqliteTelemetryRepository(database); await repository.InitializeAsync(default);
            ...
            using (var connection = new SqliteConnection("Data Source=" + database))
            {
                ...
                command.CommandText = """
                    ...
                    """;
                command.Parameters.AddWithValue("$at", Start.ToString("O")); await command.ExecuteNonQueryAsync();
            }
            var capture = new CodexServerObservation("task", CodexServerSurface.TaskUsage, null, Start, Start,
                CodexServerEvidenceParser.Contract, CodexHistoricalAnalyticsParser.Contract, ServerEvidenceState.Available, "fixture") {
                CorrelatedAccountKey = "account", TaskUsage = new(Start, ["native-thread"], [new("native-thread", "partial", "task", new(null, 2.24m, 0), [])]) };
            await repository.SaveServerEvidenceAsync(new([capture]), default);
            await repository.SaveServerEvidenceAsync(new([capture]), default);
            await repository.SaveServerEvidenceAsync(new([capture with { Id = "activity", Surface = CodexServerSurface.AccountActivity,
            ...
            await repository.SaveServerEvidenceAsync(new([capture with { Id = "failed", CollectedAtUtc = Start.AddHours(1),
                CorrelatedAccountKey = null, TaskUsage = null, State = ServerEvidenceState.Error }]), default);
            var failed = await asserted.QueryAsync(selection with { AccountKey = "account" }, default);
    ...
    public void LiveProjectionPreservesMetersWithoutForecastAndNumericArtifactsRoundTrip()
    {
        ...
        var telemetry = TelemetrySnapshot.Empty with { CapturedAtUtc = Start, QuotaSnapshots = [quota], QuotaDataFresh = true };
        var live = CodexIntelligenceProjection.Live(telemetry);
    ...
    {
        CodexLedgerEntry Row(string id, string? account, string model, string project, DateTimeOffset at) => new(id, "source", id,
            project, at, at, account, account is null ? AccountEvidenceClass.Unattributed : AccountEvidenceClass.UserDeclaredSingleAccount,
            new(id, at, at, model, "low", 10, 20, 0, 3, 2, 35));
        var rows = new[] { Row("a", "one", "m", "p", Start), Row("b", null, "m", "p", Start),
            Row("c", "two", "m", "p", Start), Row("d", "one", "other", "p", Start), Row("e", "one", "m", "p", Start.AddDays(1)) };
        var selection = new CodexSelection(Start, Start.AddDays(1), "one", "m", "p");
        var selected = CodexIntelligenceProjection.Select(rows, selection);
        ...
        Assert.NotEqual(CodexIntelligenceProjection.Manifest(selection).Id, CodexIntelligenceProjection.Manifest(selection with { AccountKey = "two" }).Id);
        Assert.NotEqual(CodexIntelligenceProjection.Manifest(selection, "old-evidence").Id, CodexIntelligenceProjection.Manifest(selection, "new-evidence").Id);
    }

    [Fact]
    public void HistoricalContractsPreserveDifferentDenominatorsUnknownsAndUnknownDimensions()
    {
        var plan = CodexHistoricalAnalyticsParser.ParsePlan("""
        ...
        const string json = """{"threads":[{"thread_id":"t","data_status":"partial","usage_source":"task","weekly_limit_percent":2.24,"balance_usage_credits":"0","groups":[]}]}""";
        var task = CodexHistoricalAnalyticsParser.ParseTasks(json, ["t"]);
    ...
    {
        var period = new CodexPlanPeriod("period", 10080, "pro", Start, Start.AddDays(7), false, 10000,
            [new("model", [new("a", 10000)]), new("surface", [new("desktop", 10000)])]);
        CodexServerObservation Row(int i, decimal used) => new(i.ToString(), CodexServerSurface.PlanHistory, null,
            Start.AddDays(8).AddHours(i), Start.AddDays(8).AddHours(i), "contract", CodexHistoricalAnalyticsParser.Contract,
            ServerEvidenceState.Available, "fixture") { CorrelatedAccountKey = "a", AccountEvidence = AccountEvidenceClass.ServerCorrelated,
                PlanHistory = new(Start.AddDays(7), Start, false, true, 0, [period with { UsedBasisPoints = used }]) };
        var result = Assert.Single(CodexProviderReconciliation.Analyze([Row(0, 9900), Row(1, 10000)], [], [],
            new(Start, Start.AddDays(9)), Start.AddDays(9)));
        ...
        Assert.Empty(CodexProviderReconciliation.Analyze([Row(0, 9900), Row(1, 10000) with { State = ServerEvidenceState.Error, PlanHistory = null }],
            [], [], new(Start, Start.AddDays(9)), Start.AddDays(9)));
    }
    ...
    {
        var trials = Enumerable.Range(0, 48).Select(i => new QuotaCostTrial(Start.AddHours(i), Start.AddHours(i + 1), Start.AddDays(7),
            i < 24 ? 5 : 10, i < 24 ? 4.5 : 9.5, i < 24 ? 5.5 : 10.5, 5, 0, 0, null, null)).ToArray();
        ...
        var result = CodexRegimeModel.Analyze(new("test", "test", "test", 48, new Dictionary<string, int>(), scores));
        if (timeExplainsShift) Assert.Empty(result.Candidates);
        else
        {
        ...
        var stable = trials.Select(x => x with { ObservedDelta = 5, LowerDelta = 4.5, UpperDelta = 5.5 }).ToArray();
        Assert.Empty(CodexRegimeModel.Analyze(new("test", "test", "test", 48, new Dictionary<string, int>(), [Score("categories", stable)])).Candidates);
    }
    private static QuotaCostScore Score(string candidate, IReadOnlyList<QuotaCostTrial> trials) => new(
        new("codex", "default", QuotaWindowKind.Weekly, "source", "account", "codex", "pro", null, 10080),
        1, candidate, 68, 20, 1, trials.Count, 1, 0, 0, 0, 0, 0, 0, 0, 0, null, false, "evaluation-only",
        new Dictionary<string, double>(), trials, []);
    ...
    {
        var trials = Enumerable.Range(0, 48).Select(i =>
        {
            var value = control == "spike" && i == 24 ? 40 : control == "periodic" ? (i % 2 == 0 ? 5 : 15) : 5;
            return new QuotaCostTrial(Start.AddHours(i), Start.AddHours(i + 1), Start.AddDays(7), value,
                control == "coarse" ? 0 : value - .5, control == "coarse" ? 20 : value + .5, 5, 0, 0, null, null);
        }).ToArray();
        Assert.Empty(CodexRegimeModel.Analyze(new("test", "test", "test", 48, new Dictionary<string, int>(), [Score("categories", trials)])).Candidates);
    }
}
