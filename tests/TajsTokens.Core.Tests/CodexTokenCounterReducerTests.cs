// Taj's Tokens | CodexTokenCounterReducerTests.cs
...
public sealed class CodexTokenCounterReducerTests
{
    ...
    {
        CodexTokenAccountingDecision decision = Reduce(Observation("first", 100, 40, 10, 4, 110));
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 40, 10, 4, 110));
        CodexTokenAccountingDecision second = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision decision = Reduce(Observation("first", 1000, 700, 100, 20, 1100, Last(30, 20, 0, 8, 2, 38)));
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("same", 100, 40, 10, 4, 110));
        CodexTokenAccountingDecision duplicate = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 40, 10, 4, 110, Last(100, 40, 0, 10, 4, 110)));
        CodexTokenAccountingDecision rateLimitOnly = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 40, 10, 4, 110));
        CodexTokenAccountingDecision unchanged = CodexTokenCounterReducer.Reduce(
            Observation("unchanged", 99, 39, 9, 3, 110, Last(1, 1, 0, 1, 1, 2)),
            first.NextState);
        CodexTokenAccountingDecision recovered = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 40, 10, 4, 110));
        CodexTokenAccountingDecision stale = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 0, 0, 0, 100));
        CodexTokenAccountingDecision stale = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 0, 0, 0, 100));
        CodexTokenAccountingDecision reset = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 80, 10, 4, 100));
        CodexTokenAccountingDecision reset = CodexTokenCounterReducer.Reduce(
            Observation("reset", 60, 45, 8, 3, 60, Last(10, 8, 0, 2, 1, 12)),
            first.NextState);
        CodexTokenAccountingDecision next = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 80, 10, 4, 110));
        CodexTokenAccountingDecision second = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision decision = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision first = Reduce(Observation("first", 100, 80, 10, 4, 110));
        CodexTokenAccountingDecision lastOnly = CodexTokenCounterReducer.Reduce(
    ...
    {
        CodexTokenAccountingDecision decision = Reduce(Observation("first", 100, 80, 10, 4, 110, Last(100, 80, 0, 10, 4, 110)));
    ...
    {
        return CodexTokenCounterReducer.Reduce(observation, null);
    }
    ...
    {
        return new CodexTokenCountObservation(id, "session.jsonl", "session", "session", Start, "model", "high", null, last);
    }
    ...
    private static CodexTokenUsageSnapshot Last(
        ...
        long total)
    {
        return new CodexTokenUsageSnapshot(input, cached, cacheWrite, output, reasoning, total);
    }
}