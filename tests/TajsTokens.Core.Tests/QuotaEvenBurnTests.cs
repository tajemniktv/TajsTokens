...
public sealed class QuotaEvenBurnTests
{
    ...
    private static QuotaSnapshot Point => new(QuotaWindowKind.FiveHour, Start.AddHours(2), 62,
        300, Start.AddHours(5), "codex", "default", "app-server");
    ...
    {
        QuotaSnapshot[] invalid = [Point with { WindowMinutes = null }, Point with { WindowMinutes = 0 },
        ...
        foreach (var point in invalid) Assert.Null(QuotaEvenBurn.FromSnapshot(point));
    }
}
