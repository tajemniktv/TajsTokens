// Taj's Tokens | TtEvaluationPersistenceTests.cs
...
public sealed class TtEvaluationPersistenceTests : IDisposable
{
    ...
    public TtEvaluationPersistenceTests()
    {
        ...
        _directory = Path.Combine(
            root?.FullName ?? throw new InvalidOperationException("Project root required."),
            ".codex",
            "temp",
            "tt-history-" + Guid.NewGuid().ToString("N"));
    ...
    public async Task OriginalBasisAndResultsSurviveRetryConflictRestartAndLaterRestatement()
    {
        ...
        TtEvaluationSnapshot original = Snapshot();
        await repository.SaveTtEvaluationAsync(original, default);
        await repository.SaveTtEvaluationAsync(original, default);
        await Assert.ThrowsAsync<InvalidOperationException>(() => repository.SaveTtEvaluationAsync(
            original with { Report = original.Report with { Methodology = "changed" } },
            default));
        TtEvaluationSnapshot revised = original with
        {
            Id = Guid.NewGuid().ToString("N"),
            RecordedAtUtc = original.RecordedAtUtc.AddMinutes(1),
        ...
        IReadOnlyList<TtEvaluationArchiveEntry> history = await reopened.GetTtEvaluationHistoryAsync("codex", "default", 10, default);
        ...
        await Assert.ThrowsAsync<InvalidOperationException>(() => reopened.SaveTtEvaluationAsync(
            original with
            {
                Id = Guid.NewGuid().ToString("N"), Report = original.Report with { Methodology = new string('x', 512 * 1024) },
            },
            default));
    }
    ...
    public async Task CorruptAndFutureRowsRemainVisibleWithoutReusingOrDeletingOriginals()
    {
        ...
        TtEvaluationArchiveEntry corrupt = Assert.Single(await repository.GetTtEvaluationHistoryAsync("codex", "default", 10, default));
        ...
        Assert.Equal(
            "unsupported-format",
            Assert.Single(await repository.GetTtEvaluationHistoryAsync("codex", "default", 10, default)).Problem);
    ...
    public async Task BasisIdentityIsVerifiedInAdditionToPayloadChecksum()
    {
        ...
        read.CommandText = "SELECT payload FROM tt_evaluation_snapshots";
        string payload = ((string)(await read.ExecuteScalarAsync())!).Replace("tt:", "wrong:", StringComparison.Ordinal);
        ...
        TtEvaluationArchiveEntry entry = Assert.Single(await repository.GetTtEvaluationHistoryAsync("codex", "default", 10, default));
    ...
    public async Task MalformedChecksummedSnapshotDoesNotHideValidNeighborsOrGetRewritten(string defect)
    {
        ...
        TtEvaluationSnapshot original = Snapshot();
        await repository.SaveTtEvaluationAsync(original, default);
        TtEvaluationSnapshot bad = original with
        {
            Id = Guid.NewGuid().ToString("N"), RecordedAtUtc = original.RecordedAtUtc.AddMinutes(1),
        };
        bad = defect switch
        {
        ...
        string payload = JsonSerializer.Serialize(bad);
        string hash = Convert.ToHexStringLower(SHA256.HashData(Encoding.UTF8.GetBytes(payload)));
        await using (var connection = new SqliteConnection($"Data Source={Database}"))
        {
            await connection.OpenAsync();
        ...
        IReadOnlyList<TtEvaluationArchiveEntry> history =
            await new SqliteTelemetryRepository(Database).GetTtEvaluationHistoryAsync("codex", "default", 10, default);
        Assert.Equal(2, history.Count);
        ...
        SqliteCommand read = check.CreateCommand();
    ...
    public async Task AdditiveMigrationFailureLeavesVersionAndExistingEvidenceRecoverable()
    {
        ...
        QuotaSnapshot quota = ComposedQuotaEvaluatorTests.TimelyData().Quota[0];
        await repository.UpsertQuotaSnapshotAsync(quota, default);
        await Execute(
            "DROP TABLE tt_evaluation_snapshots; UPDATE intelligence_schema SET version=5; CREATE VIEW tt_evaluation_snapshots AS SELECT 1;");
    ...
    private static TtEvaluationSnapshot Snapshot()
    {
        ...
        return new TtEvaluationSnapshot(
            Guid.NewGuid().ToString("N"),
            DateTimeOffset.UtcNow,
            data.CapturedAtUtc,
            "codex",
            "default",
            data.CapturedAtUtc.AddDays(-1),
            data.CapturedAtUtc,
            TtEvaluator.Evaluate(data));
    }
