// Taj's Tokens | SqliteCodexObservatoryReadModelTests.cs
...
public sealed class SqliteCodexObservatoryReadModelTests
{
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-observatory-read-");
        ...
        try
        {
            ...
            await store.UpsertAgentAsync(
                new Agent("root", "root", "Root agent", AgentRuntimeState.Completed, now.AddHours(-2), "model-root"),
                CancellationToken.None);
            await store.UpsertAgentAsync(
                new Agent("child", "child", "Needle worker", AgentRuntimeState.Completed, now.AddHours(-1), "model-child"),
                CancellationToken.None);
            await store.UpsertAgentAsync(
                new Agent("unrelated", "unrelated", "Other agent", AgentRuntimeState.Running, now, "model-other"),
                CancellationToken.None);
            ...
            await using (var connection = new SqliteConnection(new SqliteConnectionStringBuilder { DataSource = databasePath }.ToString()))
            {
                ...
                SqliteCommand seed = connection.CreateCommand();
                seed.CommandText = """
            ...
            IReadOnlyList<CodexSessionOverview> search = await readModel.SearchSessionsAsync("Needle worker", 1, CancellationToken.None);
            CodexSessionOverview selected = Assert.Single(search);
            ...
            CodexAgentTopology topology = await readModel.GetAgentTopologyAsync("child", CancellationToken.None);
            ...
            IReadOnlyList<CodexRolloutStorageSummary> storage = await readModel.GetSessionStorageAsync("child", 1, CancellationToken.None);
            CodexRolloutStorageSummary selectedStorage = Assert.Single(storage);
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-observatory-v3-");
        string databasePath = Path.Combine(directory.FullName, "telemetry.db");
        ...
        try
        {
            ...
            {
                await connection.OpenAsync(CancellationToken.None);
                SqliteCommand downgrade = connection.CreateCommand();
            ...
            SqliteCommand versionCommand = verify.CreateCommand();
            versionCommand.CommandText = "SELECT version FROM observatory_schema WHERE component = 'codex-observatory';";
            Assert.Equal(8L, (long)(await versionCommand.ExecuteScalarAsync(CancellationToken.None))!);

            SqliteCommand indexCommand = verify.CreateCommand();
    ...
    [Fact]
    public async Task Initialize_FreshDatabaseReopensWithSchemaVersion8()
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-observatory-reopen-");
        string databasePath = Path.Combine(directory.FullName, "telemetry.db");
        ...
        try
        {
            ...
            await using var connection = new SqliteConnection(new SqliteConnectionStringBuilder { DataSource = databasePath }.ToString());
            await connection.OpenAsync(CancellationToken.None);
            SqliteCommand version = connection.CreateCommand();
