// Taj's Tokens | CodexWorkloadEvidenceTests.cs
...
public sealed class CodexWorkloadEvidenceTests
{
    ...
    public void NativeEffortAndTurnIdentityAreRetainedWithoutContent()
    {
        ...
        ParsedRolloutRecord Parse(string type, object payload, int offset)
        {
            return parser.Parse(
                new RawSessionRecord(
                    ...
                    offset,
                    offset + 1,
                    JsonSerializer.Serialize(new { type, timestamp = "2026-09-01T00:00:00Z", payload })),
                state);
        }

        Parse("session_meta", new { id = Session }, 0);
        ParsedRolloutRecord context = Parse(
            "turn_context",
            new
            {
                turn_id = "turn-a",
                ...
                model = "model-a",
                effort = "max",
                developer_instructions = "private-content-do-not-retain",
            },
            1);
        ...
        ParsedRolloutRecord complete = Parse(
            "event_msg",
            new { type = "task_complete", turn_id = "turn-b", last_agent_message = "private-content-do-not-retain" },
            3);
    ...
    public void MissingSourceTimestampRemainsUnknownAndInheritedContextIsIgnored()
    {
        ...
        ParsedRolloutRecord Parse(string json)
        {
            return parser.Parse(new RawSessionRecord(state.FilePath, 0, 1, json), state);
        }

        ParsedRolloutRecord inherited = Parse("""{"type":"turn_context","payload":{"turn_id":"parent-turn","effort":"high"}}""");
        Assert.Null(inherited.WorkloadObservation);
        Parse(JsonSerializer.Serialize(new { type = "session_meta", payload = new { id = Session } }));
        ParsedRolloutRecord own = Parse(
            """{"type":"turn_context","payload":{"turn_id":"own-turn","effort":null,"reasoning_effort":"high"}}""");
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajs-workload-");
        try
        {
            string path = Path.Combine(directory.FullName, "test.db");
            var repository = new SqliteTelemetryRepository(path);
            await repository.InitializeAsync(CancellationToken.None);
            using var store = new SqliteCodexObservatoryStore(path);
            DateTimeOffset time = DateTimeOffset.Parse("2026-09-01T00:00:00Z");

            CodexTokenCountObservation Token(string? effort)
            {
                return new CodexTokenCountObservation(
                    ...
                    "model",
                    effort,
                    new CodexTokenUsageSnapshot(10, 0, 0, 5, 0, 15));
            }
            ...
            var metadata = new CodexWorkloadObservation(
                "context",
                "identity",
                "source.jsonl",
                0,
            ...
            command.CommandText =
                "SELECT count(*) || ':' || sum(reported_total_tokens) || ':' || min(reasoning_effort) FROM codex_native_token_events";
