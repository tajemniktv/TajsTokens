// Taj's Tokens | QuotaEvaluationCoverageTests.cs
...
public sealed class QuotaEvaluationCoverageTests
{
    ...
    {
        QuotaCostObservation[] rows = QuotaCostEvaluationTests.Observations(40);
        rows[5] = rows[5] with { TokenCategories = [0, 0, 0, 0, 0] };
        QuotaCostReport report = QuotaCostEvaluation.Evaluate(rows, "fixture");
        QuotaCostScore total = report.Scores.Single(x => x.Candidate == "total");
        QuotaCostScore categories = report.Scores.Single(x => x.Candidate == "categories");
    ...
    {
        QuotaCostObservation[] rows = QuotaCostEvaluationTests.Observations(40);
        QuotaCostScore original = QuotaCostEvaluation.Evaluate(rows, "fixture").Scores.Single(x => x.Candidate == "categories");
    ...
    public void ComposedCategoryTrainingReportsMissingEvidenceInsteadOfThrowing()
    {
        ...
        ComposedQuotaEvaluation result = ComposedQuotaEvaluator.Evaluate(data);
        ComposedQuotaScore category = result.Scores.Single(x => x.HorizonHours == .5 && x.CostModel == "categories");
    ...
    public void DatasetCoverageWeightsTokensAndNeverInfersTierOrTimeliness()
    {
        ...
        {
            return new CodexPredictiveTokenEvent("s", now, collected, model, effort, count, 0, 0, 0, 0, count);
        }
        ...
        var data = new CodexForecastDataset(
            [],
            [Setting("priority"), Setting(null), Setting("invented", "task_complete")],
            [
                Token(1, "model", null, now), Token(999, null, "low", null),
                Token(10, " ", "", now.AddMinutes(1)) with { NonReasoningOutputTokens = 1 },
            ],
            [],
            now,
            "fixture");
        QuotaEvaluationCoverage result = QuotaEvaluationCoverageBuilder.Build(data);
    ...
    {
        QuotaCostObservation[] rows = QuotaCostEvaluationTests.Observations(3);
        rows[0] = rows[0] with { QualityFlags = ["missing-context", "missing-context", "missing-runtime"] };
        rows[1] = rows[1] with { Cohort = rows[1].Cohort with { AccountKey = null }, QualityFlags = ["user-asserted-account-association"] };
        rows[2] = rows[2] with { HorizonHours = 2 };
        QuotaCostReport report = QuotaCostEvaluation.Evaluate(rows, "fixture");
        Assert.Equal(3, report.CohortCoverage.Count);
        QuotaCostCohortCoverage known = report.CohortCoverage.Single(x => x.HorizonHours == .5 && x.Cohort.AccountKey is not null);
        ...
        QuotaCostCohortCoverage asserted = report.CohortCoverage.Single(x => x.Cohort.AccountKey is null);
    ...
    {
        QuotaEvaluationCoverage result =
            QuotaEvaluationCoverageBuilder.Build(new CodexForecastDataset([], [], [], [], DateTimeOffset.UtcNow, "empty"));
