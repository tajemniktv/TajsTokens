// Taj's Tokens | CodexStateIndexedConsistencyTests.cs
...
public sealed class CodexStateIndexedConsistencyTests
{
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-state-ahead-");
        ...
        string threadId = "thread-state-ahead";
        string rolloutPath = Path.Combine(sessions.FullName, "state-ahead.jsonl");
        await File.WriteAllTextAsync(rolloutPath, "{}\n");
        long rolloutWriteMs = new DateTimeOffset(File.GetLastWriteTimeUtc(rolloutPath)).ToUnixTimeMilliseconds();
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-edge-only-");
        DirectoryInfo codexHome = Directory.CreateDirectory(Path.Combine(directory.FullName, ".codex"));
        DirectoryInfo sessions = Directory.CreateDirectory(Path.Combine(codexHome.FullName, "sessions"));
        string telemetryPath = Path.Combine(directory.FullName, "telemetry.db");
        string statePath = Path.Combine(codexHome.FullName, "state_5.sqlite");
        ...
        long nowMs = DateTimeOffset.UtcNow.AddSeconds(-1).ToUnixTimeMilliseconds();
        ...
        try
        {
            ...
            Assert.Equal(2, ingestion.Calls);
            AgentRelationship relationship = Assert.Single(await store.GetAgentRelationshipsAsync(CancellationToken.None));
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-state-rotation-");
        DirectoryInfo codexHome = Directory.CreateDirectory(Path.Combine(directory.FullName, ".codex"));
        DirectoryInfo sessions = Directory.CreateDirectory(Path.Combine(codexHome.FullName, "sessions"));
        string rolloutPath = Path.Combine(sessions.FullName, "older.jsonl");
        await File.WriteAllTextAsync(rolloutPath, "{}\n");
        ...
        try
        {
            ...
            CodexStateCatalogBatch? result = await new CodexStateCatalog(codexHome.FullName)
    ...
    {
        DirectoryInfo directory = Directory.CreateTempSubdirectory("tajstokens-state-malformed-");
        DirectoryInfo codexHome = Directory.CreateDirectory(Path.Combine(directory.FullName, ".codex"));
        DirectoryInfo sessions = Directory.CreateDirectory(Path.Combine(codexHome.FullName, "sessions"));
        string badPath = Path.Combine(sessions.FullName, "bad.jsonl");
        string goodPath = Path.Combine(sessions.FullName, "good.jsonl");
        await File.WriteAllTextAsync(badPath, "{}\n");
        await File.WriteAllTextAsync(goodPath, "{}\n");

        try
        {
            string newer = Path.Combine(codexHome.FullName, "state_6.sqlite");
            await CreateStateDatabaseAsync(
                newer,
            ...
            await using (var connection = new SqliteConnection(new SqliteConnectionStringBuilder { DataSource = newer }.ToString()))
            {
                ...
                SqliteCommand malformed = connection.CreateCommand();
    ...
    private sealed class CompleteIngestionService : ICodexSessionIngestionService
    {
        ...
        public Task<CodexIngestionResult> IngestAsync(string filePath, CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(
                new CodexIngestionResult(1, 1, Path.GetFileNameWithoutExtension(filePath))
                {
                    LastCompleteRecordOffset = length, SourceLength = length,
                });
        }
    }
    ...
    private sealed class PendingThenCompleteIngestionService(string sessionId) : ICodexSessionIngestionService
    {
        ...
        public Task<CodexIngestionResult> IngestAsync(string filePath, CancellationToken cancellationToken)
        {
            ...
            long length = new FileInfo(filePath).Length;
            return Task.FromResult(
                new CodexIngestionResult(Calls == 1 ? 0 : 1, Calls == 1 ? 0 : 1, sessionId)
                {
                    LastCompleteRecordOffset = Calls == 1 ? Math.Max(0, length - 1) : length, SourceLength = length,
                });
        }
    }
}