...
| Evaluation coverage | `QuotaEvaluationCoverageBuilder`, Model Lab and cost CLI; token-weighted composition, collection-time and requested-setting counts, per-cohort quality flags | Built-interval diagnostics, not complete rejected-origin accounting, token-to-account attribution or canonical duplicate coverage |
