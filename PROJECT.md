...
The production UI exposes this slice through the Codex Threads page. Its bounded catalog reads
state-backed thread rows directly (so a thread does not need a normalized rollout session to be
selectable), and its on-demand detail view renders source-native metadata, project roots, section
and pin state, directional spawn edges, dynamic tools, turn/item lanes, realtime items, source
paths, reconciliation policy, and capability/truncation diagnostics. Content-bearing item JSON is
not rendered into the summary and is never written to TajsTokens durable storage.
